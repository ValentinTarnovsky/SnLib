...
- **`flags(List.of("HIDE_TOOLTIP"))`** (1.29.0) - the second SnLib-only flag name next to `HIDE_ALL`, and not the same thing: `HIDE_ALL` expands to every real `ItemFlag`, leaving the display name on hover, while `HIDE_TOOLTIP` sets the `hide_tooltip` component so no tooltip box is drawn at all - the usual want for a filler pane or a menu background. It is never implied by `HIDE_ALL` (that would erase the name of every item already using it), so list it explicitly. Needs 1.20.5+ (`SnCompat.probe` on `ItemMeta.setHideTooltip`); on 1.20.4 it is skipped with one WARN.
