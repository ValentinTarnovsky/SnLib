...
The PURE decision core of the click listener: given what the event says, what should happen to it. Extracted so the whole routing table is exhaustively unit-testable without a server (`OfferRoutingTest`, 21 tests, most of them sweeping every `ClickType` or every `InventoryAction`). Package-private final class, private constructor, two enums and three static methods.
