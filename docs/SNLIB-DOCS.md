...
Immutable subcommand node built by `SubCommandBuilder`: `name` (trim + lowercase), `aliases` (lowercased, immutable copy), nullable `permission`, nullable `usage`, `description`, `visible`, `helpVisible` (1.10.0), `args` (immutable `LinkedHashMap`, declaration order is parse order), `requiredArgs`, `conditions`, nullable `executor`, and `children` (1.6, immutable copy; a non-empty list makes the node a GROUP that dispatches instead of running its args/executor). `static Sub of(String name, @Nullable String permission, String description, Consumer<CommandContext> executor)` fabricates the default subs (argless, visible, help-visible, no children).
