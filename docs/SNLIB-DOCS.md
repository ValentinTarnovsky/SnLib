...
- `public static void bindAliasSupplier(RootCommand command, @Nullable Supplier<Collection<String>> supplier, boolean dynamic)` (1.38.0) - the same binding plus the `dynamic()` flag, stored as `new RootState(supplier, dynamic)`; what `RootBuilder.register()` calls.
...
- `claim(known, key, command)` (1.33.1) - claims the key for the root when `known.get(key)` is null, returning the occupant that kept it or null when the claim landed. Replaces `putIfAbsent`, which does not reach the dispatcher on Paper 1.20.6+.
...
- `RootState` (private static final class; `AliasState` until 1.38.0) - per-root registration state: the `supplier`, the `dynamic` flag, the `volatile List<String> active` currently-registered alias keys and the `volatile List<String> namespaced` aliases that lost their bare key but hold their `prefix:alias` form.
...
**Notes and gotchas**
- Claiming a dynamic alias never overwrites: an alias already taken by another command is left with the existing command and reported as a collision, not operative for this root under its bare key (its `prefix:alias` form still works when free).
...
- `dynamic()` collisions are noted once per owner and key PER ENABLE (1.38.0): a collision that clears and comes back within the same enable is not noted again; the next enable (after `Sn.shutdown` swept `COLLISIONS_NOTED`) notes it afresh.
...
- `finalName`: `SnLib-${project.version}` (produces `SnLib-<version>.jar`, e.g. `SnLib-1.38.0.jar`).
...
- Handoff consistency note: the handoff mentions "114 tests"; the last count verified in this documentation was 204 tests across 21 suites (see prior revisions for the full step-by-step history). Current verified count (surefire, `mvn test`) is 652 tests, all green, on the 1.38.0 release.
