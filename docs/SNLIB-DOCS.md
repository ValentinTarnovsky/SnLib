...
- `resolve` returns a sealed `Resolution` (`Empty`, `Message`, `GroupHelp` since 1.36.0, `Run`), so the outcome is decided independently of the Bukkit context and unit-tested without a server (`NestedCommandTest`).
