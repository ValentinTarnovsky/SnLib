...
> withheld keys is reported once per boot naming the file and the key, and the misplaced-marker
> lint now runs over the disk file too (section 04).
...
4. In-memory merge: `planInsertions` + `applyInsertions` over the disk lines; with `prune=true` `prune` is applied as well. `logFrozenByDiskMarker` (1.19.0) then logs one WARN per owner-declared marker that actually withheld keys - "[update-configs] <file>: N key(s) not inserted because the file declares sn:extensible at '<path>'", or "... because the file header declares sn:extensible-root" for the root - and nothing when a marker withheld nothing. If the result is identical to disk, it returns without touching anything (no backup, no gate, no log).
