...
- `public RootCommand register()` - builds the tree, resolves the `fallbackSub` name (1.37.0; `IllegalStateException` when it matches no declared root-level subcommand by name or alias, or names a group, checked before anything is registered), injects the applicable defaults, binds the alias supplier to the built root (`BukkitCommandRegistry.bindAliasSupplier`, 1.5.0) and registers it against Bukkit; returns the `RootCommand`.
...
- `detach(RootCommand)` - unhooks the command from whichever path registered it: removes BY KEY (1.33.1) each active alias and its `owner:alias` form plus the root name and its `owner:name` form, each via the two-arg `known.remove(key, command)` so an entry owned by something else is never touched; calls `command.unregister(map)`; and if the declared `PluginCommand`'s executor is a `PluginCommandAdapter` of THIS root, clears the executor and tab completer. Resets `AliasState.active` but KEEPS the supplier binding, so the same instance can be detached and registered again without silently losing its config-driven aliases. Returns whether the map was mutated.
...
**Notes and gotchas**
- Claiming a dynamic alias never overwrites: an alias already taken by another command is left with the existing command and reported as a collision, not operative for this root.
...
- `static boolean warnsUndeclared(@Nullable Collection<String> supplied, Collection<String> added)` (1.12.1) - whether the added aliases deserve the "not declared in the plugin.yml" WARN: true only when `supplied` is null (fallback path) AND something was added. A non-null `supplied` means an authoritative source owns the aliases at runtime, so declaring them in the plugin.yml is impossible and the WARN would be boot noise.
...
- Dynamic aliases are claimed, never overwritten: if another command already owns the alias key in the CommandMap, the alias is not operative for the root and the collision is reported as a WARN. Since 1.33.1 a collided alias is NOT recorded as active, so the next register pass retries it (an alias whose owner disappeared starts working after a reload) and re-warns while it stays taken.
...
- `finalName`: `SnLib-${project.version}` (produces `SnLib-<version>.jar`, e.g. `SnLib-1.37.0.jar`).
...
- Handoff consistency note: the handoff mentions "114 tests"; the last count verified in this documentation was 204 tests across 21 suites (see prior revisions for the full step-by-step history). Current verified count (surefire, `mvn test`) is 634 tests, all green, on the 1.37.0 release.
