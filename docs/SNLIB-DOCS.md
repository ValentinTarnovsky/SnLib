...
- `public @Nullable GuiItemDef itemAt(int slot)` - The definition rendered at `slot` for this viewer with precedence: API bind, then paginated entry, then the slot's declared item. Null for an empty slot, which since 1.16.1 also covers a definition hidden from this viewer by its `view-requirements` and a paginated slot the current page left empty (before, both returned a definition the viewer could not see; the paginated case even fell through to the declared item underneath). Resolved by the private `renderedAt(int)`, the same helper `handleClick` uses, so the query and the click can never disagree.
...
- `public void handleClick(int slot, ClickType click)` - Click dispatch invoked by the shared click listener with a top-inventory raw slot: it resolves what the slot actually SHOWS this viewer through `renderedAt(int)` (manual bind, paginated entry, declared item, each behind its own view requirement), skips disabled navigation items and delegates to `runClick`, which resolves the definition's per-click matrix (actions, requirement and deny by `ClickType`, specific-over-generic field by field) and applies the menu's opt-in strict-clicks gate, with this session as the page target and the click type in the context. No-op if the session is closed. **View requirements gate interaction since 1.16.1**: a slot that renders nothing for this viewer fires NOTHING - not the click actions, not the deny actions - so `view-requirements` never need duplicating into `click-requirements`. Until 1.16.0 the dispatch resolved the definition by slot without re-testing the view requirement, so an item hidden from a player was still clickable on its empty slot (and an empty paginated slot fell through to a declared item underneath).
