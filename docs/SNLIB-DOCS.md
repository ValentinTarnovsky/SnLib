...
> Updated on 2026-08-30 for the 1.33.1 fix (no API level bump, no public surface change):
> dynamic command aliases reach Bukkit again. On Paper 1.20.6+ `getKnownCommands()` returns a
> view over the Brigadier dispatcher that forwards only its OVERRIDDEN methods, and the
> `putIfAbsent` this layer used is not one of them - every config-driven alias was written to a
> dead table and reported as a successful insert, so it existed in neither form, not even at
> cold boot. Registration now claims keys through the map's own `get`/`put`, teardown removes by
> key (the entry-set iterator there refuses removal), a collided alias is no longer recorded as
> active, the alias binding survives a detach, and the new internal `CommandSync` re-syncs the
> server dispatcher after a mutating pass so the reload path also works on 1.20.4 / 1.20.5
> (section 13).
> Updated on 2026-08-27 for the 1.33.0 change (no API level bump, tag surface only): `[rgb]`
> and `[small]` became SPAN tags. They also open mid-line, and the new `[/rgb]` / `[/small]`
> close them, MiniMessage-style; an unclosed span runs to end of line so the historical prefix
> form renders byte-identical. Each `[rgb]` span receives the COMPLETE 7-anchor gradient over
> its own visible characters, and closing it restores the legacy color/format codes active
> outside the span (or `&r` when there were none). A stray close tag is consumed silently;
> `[center]` / `[noprefix]` stay prefix-only. `StylePolicy` gates the mid-line `[rgb]` form
> under the same `GRADIENT` capability, and `strip` drops or keeps `[rgb]`/`[/rgb]` together
> (sections 03, 04).
...
> CONTROL_DROP alike), and the `strict-clicks` gate now lets a DROP through when the clicked
> item declares that list - NUMBER_KEY, SWAP_OFFHAND and UNKNOWN stay always discarded, and an
> item with no drop list resolves a Q press exactly as it did in 1.30.0 (section 12).
...
> Updated on 2026-07-27 for the 1.19.0 change (no API level bump, behavior only): the
> `# sn:extensible` / `# sn:extensible-root` markers are now honored when the DISK file
> declares them, not only the jar resource, so a server owner can freeze a section the author
> still manages. The rule is OR, so protection only ever adds: a resource-declared section
> stays declared even with the comment deleted on disk. An owner-declared marker that actually
> withheld keys is recorded at FINE naming the file and the key, and the misplaced-marker lint
> now runs over the disk file too (section 04).
> Updated on 2026-07-27 for the 1.19.1 fix: two warnings that fired forever on deliberate
> configuration are gone. The owner-declared marker record dropped from WARNING to FINE (the
> resource keeps shipping the frozen entries, so the condition never clears and the warning
> repeated on every boot with nothing to act on), and an item whose `key:` letter the owner
> removed from the `layout:` is now hidden silently instead of costing two WARNs per button
> per boot (sections 04 and 12).
> Updated on 2026-07-27 for the 1.20.0 change (API level 12): runtime layout regions. A menu
> declares named groups of cells under `regions:` (a scalar is shorthand for `key:`, the long
> form takes `slots:` or `key:`) and `GuiSession.bindEach(String, List, BiConsumer)` lays one
> entry per cell over them, the new `GuiEntry` picking that entry's template AND placeholders
> per cell - which is what lets state variants (allowed/denied) share one region. Entry i goes
> to cell i, so cardinality, order, position and existence of the region all move out of Java
> and into the file; `GuiDef.regionSlots(String)` reports the owner's cell count. Ownership is
...
> Updated on 2026-08-11 for the 1.25.0 addition (API level 17): placeholders that need no
> player. `ExpansionBuilder.global(String, Supplier)` and `globalPrefixed(String, Function)`
> bind a resolver that takes no `OfflinePlayer`, so a strictly null requester - a global
> hologram, a Discord bridge, a console task, `/papi parse --null` - is answered instead of
> dropped; `BuiltExpansion.onRequest` now locates the resolver BEFORE rejecting a null player,
> and the player-bound `placeholder`/`prefixed` bindings keep the short-circuit unchanged.
> `LeaderboardCache.exposePlaceholders` binds its `top_` tokens global (sections 14 and 18).
...
- Architectural contract documented in the class Javadoc: (a) shared classloader with owner-keyed registries, (b) `*.internal` packages outside semver, (c) 1.20.4 to 1.21.8 compatibility with `Sound`/`Particle`/`ItemFlag` treated as open sets (never switch/EnumSet over them), lenient aliases with WARN (`HIDE_POTION_EFFECTS` -> `HIDE_ADDITIONAL_TOOLTIP`, `REDSTONE` -> `DUST`) and every post-1.20.4 API only behind `SnCompat.probe`, (d) frozen entrypoint: `SnPlugin` + `requiredApiLevel()` + `SnSpec` + `SnApi.LEVEL` only grow within a major version (nothing in them is ever removed or changed incompatibly; 1.38.0 added `onPreEnable()`).
...
Mandatory base class of every SnLib consumer and the ONLY initialization path of the library: since `SnLib.init` is package-private, extending this class is the only public way to obtain a context. Part of the frozen entrypoint (this surface only grows within a major version). Consumers must declare `depend: [SnLib]` in their plugin.yml. `abstract` class extending `JavaPlugin`; `onEnable()` and `onDisable()` are `final`. Enable order: handshake, `onPreEnable()` (1.38.0), `SnLib.init` (the first step that writes into the plugin folder), `onInnerEnable()`.
...
- `public final void onEnable()` - first clears `sn` (1.38.0: a context left from an earlier enable of the same instance, a plugin manager disable + enable, is already shut down and must not be torn down again by a refused re-enable), then runs the API level handshake; if it passes, runs `passesPreEnable()` (1.38.0, see `onPreEnable()` below) and returns on a refusal; otherwise does `this.sn = SnLib.init(this, buildSpec())` and then calls `onInnerEnable()` inside a `Throwable` try/catch. The catch branches on `isEnabled()`: still enabled means nobody disabled anything and the failure is unexpected, so it logs SEVERE "onInnerEnable failed" WITH the stacktrace and disables the plugin via `PluginManager.disablePlugin(this)`; already disabled means the consumer disabled itself inside `onInnerEnable()` and threw only to cut the enable short, so it logs the single SEVERE line `"Enable aborted: " + reason` (the message of the throwable, or its simple class name when there is none), puts the throwable at FINE and returns without disabling again. On a normal return the same `isEnabled()` check runs before `sn.commands().applyLang()`, so the `return`-style abort skips the lang pass instead of seeding a command tree the teardown already removed.
- `public final void onDisable()` - (1.38.0) FIRST returns when `sn == null`: no context means the enable stopped before `SnLib.init` returned (a failed handshake, a refused `onPreEnable()`, an init that threw), `onInnerEnable()` never ran and there is nothing to undo, so `onInnerDisable()` is NOT called. The check is the first statement because a `disablePlugin(this)` issued inside `onPreEnable()` runs `onDisable()` synchronously, before the hook returns (a flag set after the hook would come too late). Behavior change: before 1.38.0 a failed handshake or a throwing init still called `onInnerDisable()`. With a context it opens the teardown window with `sn.beginTeardown()` BEFORE anything else, then calls `onInnerDisable()` and, in a `finally` (that is, even if the consumer's disable throws), runs `sn.shutdown()`. Opening the window first is what makes the consumer's final flush inside `onInnerDisable()` count as shutdown work: `SnYml.save()` writes inline and a main-thread `SnFuture.join()` is accepted instead of logging the out-of-shutdown WARN. Every module is still live at that point (the pool is open, joins complete normally); only the window flag moved earlier, never the teardown itself.
...
- `protected boolean onPreEnable()` (1.38.0, API level 25) - gate that runs after the handshake and BEFORE `SnLib.init`, so before SnLib creates `config.yml`, `lang/` or `guis/`. Default `return true`. `sn()` is null inside it: log through `getLogger()`; the hook may write files of its OWN (the `license.yml` a license gate seeds), never the ones SnLib manages. `false` refuses: the plugin ends disabled without a context, no SnLib file is written, neither `onInnerEnable()` nor `onInnerDisable()` runs, and the library adds no log line (the hook logged its reason; Paper logs "Disabling ..."). A hook that disabled the plugin is a refusal whatever it returned. A throw follows the `onInnerEnable()` rules: plugin already disabled -> one SEVERE `Enable aborted: <reason>` plus the throwable at FINE; still enabled -> SEVERE "onPreEnable failed" with the stacktrace, then `disablePlugin(this)`. The decision is the pure package-private `static PreEnableOutcome preEnableOutcome(boolean threw, boolean returned, boolean stillEnabled)` (`PROCEED`, `ABORT_QUIET`, `ABORT_ONE_LINE`, `FAIL_TRACE_AND_DISABLE`, `DISABLE_QUIET`) applied by the private `passesPreEnable()`; covered by `SnPluginPreEnableTest`. The license gate of a licensed consumer belongs here (`return LicenseManager.init(this, "myplugin");`), so an unlicensed install shows only the license file. The LEVEL 25 handshake keeps such a consumer off an older SnLib that would never call the hook.
- `protected abstract void onInnerEnable()` - the consumer's enable logic; runs after the handshake, `onPreEnable()` and the context initialization. A consumer that refuses to run (invalid license, missing requirement) disables itself with `disablePlugin(this)` and then leaves this method by `return` OR by throwing: both are deliberate aborts and both cost exactly one log line (`Enable aborted: <reason>`), never a stacktrace. Since 1.20.3; before it, the throwing form - the shape the licensed plugins use - printed a full SEVERE stacktrace right after the gate's own clean `[Sn] License: FAIL` line.
- `protected void onInnerDisable()` - the consumer's disable logic; runs before the context teardown, and only when the plugin got a context (1.38.0). Optional (empty default implementation).
- `public final Sn sn()` - this plugin's SnLib context; available from `onInnerEnable()` onward (null inside `onPreEnable()`).
...
Per-plugin SnLib context: the handle through which a consumer reaches every module it declared in its `SnSpec`. `final` class; package-private constructor `Sn(JavaPlugin plugin, SnSpec spec)` that wires ALL modules at construction (declared ones end up mounted; undeclared ones stay `null` only for the 5 gated modules). Part of the frozen entrypoint surface: its accessor set only grows within a major version. Field `volatile boolean shuttingDown` (package-private): the teardown WINDOW flag, set before anything else by `beginTeardown()` (or by `shutdown()` when nothing opened the window first); it flips `SnYml.save()` to synchronous writes and makes `SnFuture.join()` accept the calling thread. Field `private volatile boolean shutdownRan`: the separate idempotency guard of `shutdown()`. Method `void beginTeardown()` (package-private): opens the window WITHOUT running the teardown, called by `SnPlugin.onDisable()` right before `onInnerDisable()` so the consumer's final flush counts as shutdown work.
...
If `shutdownRan` was already true, it returns without doing anything (idempotency). Note the guard is the dedicated `shutdownRan` field, NOT `shuttingDown`: the teardown WINDOW opens earlier than the teardown itself (see `beginTeardown()`), so guarding on the window flag would turn the whole teardown into a no-op. Otherwise it runs exactly:

- Step 0 (pre-step): `shutdownRan = true` and `shuttingDown = true` BEFORE everything else: `SnYml.save()` switches to inline writes and `SnFuture.join` accepts the teardown thread. This way every persistence done inside the teardown itself (equipment backups, debug toggles, data files) writes SYNC on the calling thread instead of going through a scheduler about to be cancelled. `shuttingDown` is normally already true here, set by `beginTeardown()` before `onInnerDisable()` ran; the assignment is idempotent and keeps a direct `sn.shutdown()` call correct on its own.
...
- `public boolean commandPriority()` (1.39.0, API level 26) - whether the plugin declared command priority (see `Builder.commandPriority()`).
...
- `public Builder commandPriority()` (1.39.0, API level 26) - declares the plugin the core of its server mode: every root it registers through `sn.commands()`, dynamic or declared in the plugin.yml, takes the bare name and the aliases from the commands of OTHER plugins, registered before or after it; the displaced command keeps its namespaced form (`/essentials:money`) and gets the bare key back when the root unregisters while its plugin is still enabled. Only the aliases SnLib manages (builder, supplier, config); the `aliases:` of a plugin.yml command stay Bukkit's. Never taken: a key of the same plugin, a key holding `:` (a namespaced form), a vanilla/Bukkit/Paper command, a command registered through Paper's Brigadier API, a `commands.yml` alias, or a root of another plugin that declared priority too (whichever holds the key keeps it). Threaded into `new SnCommands(ctx, lang, debugCommand, commandPriority)` by `Sn`; the mechanics live in `BukkitCommandRegistry` (section 13).
...
- `public static String applyPrefixTags(String line)` - Consumes the `[center]`, `[rgb]`, `[small]` and `[noprefix]` prefix tags at the start of the line, in any order and case-insensitive (via `regionMatches(true, ...)`), in a loop until none remain (one branch per tag). `[noprefix]` (1.9.0) is stripped WITHOUT effect here: it is SnLang's marker for "do not prepend the configured prefix", consumed by every render so the literal tag never reaches the player. Since 1.33.0 `[rgb]` and `[small]` are SPAN tags handled by the private `applySpans` scanner: they also open mid-line, `[/rgb]` / `[/small]` close them, an unclosed span runs to end of line (the prefix form renders byte-identical, and a `'['`-free body takes a fast path that reproduces the old whole-line code exactly), each `[rgb]` span receives the COMPLETE 7-anchor gradient over its own visible characters, a stray close tag is consumed silently, and `[center]`/`[noprefix]` stay prefix-only (mid-line they render literally). Closing a gradient span re-emits the outer legacy style tracked by the private `OuterStyle` holder (last active color code plus the format codes opened since it, vanilla clear semantics), or a bare `&r` when the outside carried no legacy code - a MiniMessage-tag outer style is NOT tracked and falls back to that `&r`. FIXED internal application order per segment: `[small]` runs first (delegates to `SmallCapsUtil.applySmallTag` per constant-state segment, so the tags themselves are never glyph-mapped), then `[rgb]` (contiguous rgb-on text, which may cross `[small]` boundaries, is gathered into one run handed to `RgbGradientUtil.applyRgbTag`); `[center]` is re-emitted as a single normalized leading `[center]` mark that the final legacy phase consumes. Small BEFORE rgb so the gradient colors the final glyphs and the small pass operates on the short string (not on the string inflated 9x by the gradient hex); since the mapping is 1:1 and does not touch spaces, the gradient's visible-count does not change and the tag permutations render identically. Null returns null.
...
- `public enum Capability` - the styling forms a policy can allow: `LEGACY_COLOR` (`&0`-`&f`), `HEX` (`&#RRGGBB`), `BOLD`/`ITALIC`/`UNDERLINE`/`STRIKETHROUGH`/`OBFUSCATED` (`&l &o &n &m &k`), `MINIMESSAGE` (cosmetic tags as a whole - a `<red>` or `<bold>` is MiniMessage usage, not legacy usage) and `GRADIENT` (the `[rgb]` span tag in any position plus the `<gradient>`/`<rainbow>` tags).
...
- `public String strip(String input)` - removes the disallowed styling while keeping the allowed styling and all visible text. Section codes are normalized to `&` first (`SnText.normalizePapiOutput`); non-cosmetic MiniMessage tags are always removed; `&r` and `<reset>` are always kept. The leading `[center]`/`[rgb]`/`[small]` prefix run is preserved, dropping `[rgb]` only when `GRADIENT` is disallowed; since 1.33.0 the mid-line span forms `[rgb]` and `[/rgb]` are dropped or kept together under the same toggle (`[small]`/`[/small]` are never gated).

#### Internal logic (detection)
- `analyze(String)` returns a `Usage` struct (per-form booleans, splitting `cosmeticMini` from `nonCosmeticMini` for the subset rule) by scanning the normalized input once, skipping `\<` escapes and consuming the leading prefix-tag run (`[rgb]` sets gradient usage; since 1.33.0 a mid-line `[rgb]` sets it too, while a bare `[/rgb]` applies nothing and is not usage).
...
State: `saveLock` (a monitor) protects `pendingSnapshot`, `pendingSeq`, `writeScheduled` and the `saveSeq` counter; `ioLock` (a `ReentrantLock` since 1.24.1) protects the physical write and `lastAttemptedSeq`. The only lock nesting anywhere in the class is `ioLock` -> `saveLock`, in `flush()` and `drainPendingWrites()`; `save()`'s teardown branch releases `saveLock` BEFORE calling `writeToDisk`, so nothing ever holds `saveLock` while waiting for `ioLock` and no inversion exists.
...
3. Normal runtime: under `saveLock` it replaces `pendingSnapshot`/`pendingSeq` (coalescing: at most ONE pending write per file; a newer save overwrites the pending snapshot) and, if no drain is scheduled (`!writeScheduled`), schedules `ctx.scheduler().supplyAsync(this::drainPendingWrites)`. A `whenComplete` resets `writeScheduled = false` ONLY if the future ended with an error (if the async never ran, a later save can reschedule); on the happy path the drain itself resets it. The future is not retained: nothing joins it (see `flush()` below).
4. `drainPendingWrites()` (runs on the async pool): a loop that holds `ioLock` across BOTH the take and the write. Under `saveLock` it steals the pending snapshot and clears it; if there is nothing, it turns off `writeScheduled` and finishes; if there is, it writes via `writeToDisk` (a reentrant re-acquisition) and iterates again, thus consuming saves that arrived while it was writing. The `ioLock` span is load-bearing as of 1.24.1: were the take outside it, the drain would hold the only copy of a snapshot with `pendingSnapshot` already null, and a concurrent `flush()` would see nothing staged plus a free lock and return before the bytes landed.
...
`flush()` (rewritten in 1.24.1): it does NOT join the scheduled write's future. It acquires `ioLock` with `tryLock(FLUSH_BUDGET_SECONDS = 10, SECONDS)`, then drains `pendingSnapshot` to empty in a loop, writing each snapshot via `writeToDisk` with its `pendingSeq`, and unlocks only if it actually acquired. Invoked by the context teardown (via `YmlManager.flushAll()`) so no coalesced write is ever lost.

Two waits are involved and only one is bounded, which is the design:

- **Somebody else's in-flight write** is a courtesy wait, so it is capped at 10s. An intrinsic monitor cannot be acquired with a timeout, which is why `ioLock` is a `ReentrantLock`: a wedged disk (stalled NFS/SMB mount, host snapshot, AV filter) would otherwise hang `Sn.shutdown()` before `db.shutdown()`, so every plugin after this one in the disable order never gets `onDisable`.
- **The snapshot this instance staged** is the caller's data, never a courtesy. It is always written, blocking uninterruptibly inside `writeToDisk` if the disk is wedged - which is exactly what the pre-1.24.1 code did after its own timeout expired.

`InterruptedException` from `tryLock` re-arms the flag and falls through to the drain rather than returning: the flag survives, so every later file in `flushAll()`'s loop would take the same branch and returning would silently drop all of their staged writes. `tryLock` throws at nanosecond zero when the flag was already set, so that path is NOT warned about - warning there would print one false alarm per mounted file on a shutdown where every write landed cleanly. Only a genuine 10s expiry logs `"A write to <file> has been in progress for over 10s"`.

**Why the join went away.** A consumer that called `save(); flush();` on the primary thread saw the old `get(10, TimeUnit.SECONDS)` expire its full budget on every single call on a live 1.21.8 Paper server, freezing the server for ten seconds a time and tripping the watchdog; the write only landed because `flush()` fell through to the inline write after the timeout. Taking the staged snapshot over reaches the same end state with no wait: the scheduled drain then finds nothing staged and exits. Note that `save()` alone was, and remains, the complete persist path at runtime - `flush()` is a teardown primitive and consumers should not pair the two.
...
- `public static List<String> markerWarnings(List<String> lines)` (1.15.0) - pure lint of a yml, resource or disk file alike: one human-readable line per `sn:extensible` marker placed on a key that holds a plain value (protects nothing, always a mistake). Empty sections `{}` / `[]` are legitimate and never reported.
...
Owner-owned sections (1.15.0): `Node.extensible` is set at parse time when the node's comment run contains a line that is exactly `# sn:extensible` (leading `#` and whitespace stripped, case-insensitive, the token owning its line), and on the ROOT node when the file header - the lines before the first key - contains `# sn:extensible-root`. Nothing below an extensible node is inserted or pruned: the whole subtree is server-owner data. The marked key itself is still schema and is re-inserted whole when the disk file lacks it; an empty section (`points: {}`) is preserved as-is. `markerWarnings(lines)` (public, pure) returns one line per marker placed on a key that holds a plain value, which protects nothing and is always a mistake; `{}` and `[]` are legitimate empty catalogues and never reported.

Either side may declare a marker (1.19.0), and the two only ever ADD protection - there is no way to subtract it:

- **Resource-declared** (`resource.extensible`): the author states the entries are the owner's. `collectInsertions` and `collectRemovals` both return immediately, silently. Binding: deleting the comment from the disk copy does NOT re-enable merging, which is the anti-tamper property the 1.15.0 contract promised.
- **Disk-declared** (`disk.extensible`): the owner freezes a subtree the author still manages. `collectRemovals` returns immediately as before, but `collectInsertions` does NOT return - it sets a `frozenAt` path and keeps walking, counting each block it would have inserted into a `Map<String,Integer> frozen` keyed by that path instead of emitting an `Insertion`. The outermost marker owns the count (the `frozenAt == null` guard), since that is the one the owner actually typed; the empty-string key means the root, i.e. a `# sn:extensible-root` in the disk header. `planInsertions` has two forms: the no-arg one used by the pure public `merge` discards the map, the `Map` overload used by `update`/`updateFromLines` hands it to `logFrozenByDiskMarker`.

Walking instead of returning is what makes the disk case reportable: a marker that withholds nothing produces no map entry and therefore no log line, so a file frozen after it was already complete stays silent. Before 1.19.0 the disk parse set the same flag and it was never read, so a marker typed on disk was inert.
...
3b. `logDiskMarkerWarnings` (1.19.0): the same `markerWarnings` lint run over the DISK lines, minus every finding the reference already produced, so a misplaced marker the owner typed surfaces too without reporting a shipped authoring mistake twice per file.
4. In-memory merge: `planInsertions` + `applyInsertions` over the disk lines; with `prune=true` `prune` is applied as well. `logFrozenByDiskMarker` (1.19.0) then records one line per owner-declared marker that actually withheld keys - "[update-configs] <file>: N key(s) not inserted because the file declares sn:extensible at '<path>'", or "... because the file header declares sn:extensible-root" for the root - and nothing when a marker withheld nothing. It logs at FINE, not WARNING (changed in 1.19.1): freezing a section in order to delete entries permanently is the POINT of the marker, and the resource keeps shipping those entries forever, so the condition never clears - a warning would repeat on every boot for a deliberate choice with nothing to act on. If the result is identical to disk, it returns without touching anything (no backup, no gate, no log).
...
`updateFromLines` follows the same flow with three differences: the reference is in-memory lines (no step 1), it returns early with `false` if there are no insertions (no prune), and it returns a boolean indicating whether the disk changed. `logFrozenByDiskMarker` runs BEFORE that early return on purpose: a marker that withheld every pending insertion leaves the list empty, and that is exactly the case worth reporting. Two overloads: `updateFromLines(JavaPlugin, ...)` and `updateFromLines(Logger, ...)`. The `Logger` variant (1.5.0) exists for callers that hold a reference in memory but no `JavaPlugin` handle - the GUI seeder reads each `guis/*.yml` reference straight from the consumer jar and merges it through this same path; the `JavaPlugin` overload just delegates to it with `plugin.getLogger()`.
...
- The 1.19.0 disk-side rule is OR, never AND, and that is what keeps the 1.15.0 anti-tamper promise intact: a disk marker can only ADD protection, so an author-declared section stays declared whatever the disk file says. The lang module is the reason the old "only the resource declares it" wording was already inaccurate - `mergeTranslation` passes the on-DISK `messages_en.yml` as the reference, so a marker typed there has always been honored. Whether a hand-typed marker counted used to depend on which file happened to be the reference; now it counts everywhere.
- Reading code must default every key regardless of markers. An owner can freeze any section of any file, so "the merge guarantees this key exists on disk" was never a safe assumption and is now visibly not one.
...
#### MergeBackups (internal, 1.39.0)
`src/main/java/com/sn/lib/yml/internal/MergeBackups.java`

Names of the backups `YamlUpdater` writes before a merge, `old-<base>-<yyyyMMdd-HHmmss>.yml`, built on one stamp so the pruning and the menu listers never drift apart. `public static final String STAMP` - the regex of the stamp and extension, `\d{8}-\d{6}\.yml`; `YamlUpdater.pruneOldBackups` keeps its own exact, case-sensitive match `Pattern.quote("old-<base>-") + STAMP` (a loose prefix would mix in `config-extra`'s backups with `config`'s). `public static boolean isMergeBackup(@Nullable String fileName)` - any backup, `(?i)old-.+-` + `STAMP`, what `GuiManager` skips; `old-town.yml`, `oldies.yml` and `old-visibility-2026.yml` are not backups. Tested by `MergeBackupsTest`.
...
- `public void broadcast(String key, Ph... phs)` - Broadcast to the whole server via `Bukkit.getServer()` as an Audience; PAPI resolves against the server (null viewer), so a player-bound expansion placeholder stays literal.
- `public void broadcast(String key, @Nullable Player subject, Ph... phs)` (1.34.0) - Broadcast ABOUT a player: same Audience, but PAPI resolves against the `subject` once, so every recipient sees the subject's values (a rank prefix, a clan tag). The token may sit in the line or arrive inside a `Ph` value, since locals resolve before the PAPI pass. A null subject is the plain overload. Not for per-viewer values: loop `send(recipient, ...)` for those.
...
- `public Component get(String key, @Nullable Player viewer, Ph... phs)` - (1.23.0) first line rendered FOR a viewer, on the CALLING thread. The difference from the viewer-less overload is PlaceholderAPI: that one has no viewer and leaves `%token%` literal, while every `send` resolves it but only because it knows its target. This overload covers the case in between - a fragment SPLICED into something else (a tag inside a chat line, a lore line built into a stack) that must still carry the viewer's placeholders. Resolution runs through `SnPapi.applyHere`, so it works off the primary thread, which is the reason it exists: chat is assembled on Paper's async chat thread and has no later main-thread point to resolve at. Never cached, since the result is per viewer. Read `applyHere` before using it - passing a viewer here is the caller stating that resolving on this thread is what they want.
- `public List<Component> getList(String key, @Nullable Player viewer, Ph... phs)` - (1.23.0) list overload of the above; a missing key yields the marker line.
...
- `public void reload()` - Re-runs the seed, the merges and the re-read of both language files from disk, rebuilding all caches. Invoked only from onEnable and the reload command. Every folder registered through `addSource` is re-read in the same pass.
- `public void addSource(String folder, String namespace)` (1.36.0) - Registers an extra language folder, e.g. `addSource("modules/party/lang", "party")`, so a modular consumer keeps each module's messages next to the rest of that module's files. The folder's `messages_en.yml` is seeded from the jar resource `<folder>/messages_en.yml` and always-merged afterwards (gated by `update-configs`), exactly like `lang/messages_en.yml`; when the configured `lang` is not `en`, `<folder>/messages_<code>.yml` is merged against THAT English file and used, and a folder without the translation falls back to its own English with ONE WARN (never one WARN per key). Every key is served under `"<namespace>.<key>"` (`send(player, "party.join")`), gets the global `prefix` like any single-line message, and loses to a key of `lang/` with the same path (one WARN per key, `Key 'party.join' from modules/party/lang is already defined; the first definition wins`). A key that would have to pass through a message of `lang/` (`party.menu` is a message there and the folder brings `menu.title`) is a collision too, so a global message is never turned into a section. A namespace that is, or passes through, a message key of `lang/`, in English or in the active language, skips the whole folder with one WARN. A folder with no `messages_en.yml` in the jar nor on disk is skipped with one WARN. The folder is remembered and re-read on every reload. Registering a NEW folder only reads that folder and caches its keys (O(keys of the folder)); registering the same folder again reloads the whole module. Folder: `\` normalized to `/`, empty and `.` segments dropped, `..` resolved; blank, holding `:`, climbing out of the data folder or resolving to `lang` rejected (shared `yml.internal.ResourceFolders`); namespace: non-blank, no whitespace, no leading/trailing `.`; both throw `IllegalArgumentException`. Main thread only.
- `public void removeSource(String folder)` (1.36.0) - Forgets a folder registered through `addSource` and reloads the module, so its keys stop resolving. An unknown folder is ignored; the files stay on disk.
...
- `private void load()` - Orchestrates the load: clears `warnedKeys`, seeds the English, merges the snlib keys, parses the fallback, decides the desired code (if it is `en`, active = fallback; otherwise it loads the translation), lays every registered `addSource` folder over the result (1.36.0), caches the prefix, rebuilds the caches and finally runs `warnLiteralPrefixToken` (it scans every active key, so after a full load the keys of the extra folders too, while its WARN still names `lang/messages_<code>.yml`) and `warnInteractiveTagDrift` (it compares only the jar default of `lang/messages_en.yml`).
- `private Set<String> overlaySource(String dirPath, String ns)` (1.36.0) - Seeds and merges one extra folder (`YamlUpdater.update` for `<folder>/messages_en.yml`, `mergeTranslation` for `<folder>/messages_<activeCode>.yml`) and lays its keys over the loaded fallback and, when it is a different object, the active language. Runs after the `lang/` files resolved, so `activeCode` is final. Returns the full keys it added.
- `static Layered layerKeys(YamlConfiguration target, YamlConfiguration source, String ns)` (1.36.0) - Pure layering: copies every leaf of `source` into `target` under `ns`. A full path the target already holds, or one that would have to pass THROUGH a message of the target (`party.menu` is a message and the source brings `menu.title`), keeps the target untouched and is reported as a collision: a nested `set` would otherwise turn that message into a section and silently drop it. A namespace that is, or passes through, a message of the target leaves it untouched and reports the clash (`namespaceBlocked`). `overlaySource` rules out the clash against BOTH the English fallback and the active language before layering anything, so a clash in the translation alone cannot half-apply a folder. Package-private and unit-tested (`SnLangSourceTest`), together with `normalizeFolder` and `requireNamespace`.
- `private void cacheKey(String key)` (1.36.0) - Caches one key's resolved template lines plus its pre-rendered components when static; `buildCaches` calls it per key and `addSource` calls it for the keys a new folder added.
...
| `snlib.help.group-header` | `{group}`, `{path}`, `{description}`, `{plugin}` | Header of the own help of a `groupHelp()` group (`/<cmd> <group> help`); `{group}` is the group name, `{path}` its full command path under the typed label, `{description}` its description (v1.36.0, section 13). |
| `snlib.help.group-footer` | `{page}`, `{total}`, `{path}` | Printed after a group's help entries only when they span several pages; `{path}` is the group's full command path (v1.36.0, section 13). |
...
- `public SnFuture<String> applyOnMain(@Nullable Player viewer, String text)` - (v1.1) async-safe bridge to `apply`. On the primary thread it resolves INLINE and returns an already-completed future (`SnFuture.wrap(ctx, CompletableFuture.completedFuture(...))`); off it, it schedules a hop to the main thread via `ctx.scheduler().sync(...)` and completes the future there. Fail-open: a resolver error (`Throwable` try/catch with a debug note) or a scheduling failure (`IllegalPluginAccessException`: plugin disabled before the hop) complete with the ORIGINAL unresolved text; null text completes with null. Canonical consumption: `thenSync(...)`, just like db futures. Since 1.27.0 the off-main branch returns `SnFuture.wrapMainCompleted` rather than `wrap`, because only the scheduled main-thread task can ever complete it: a future produced off-main and then waited on FROM the main thread used to deadlock the server silently and now throws `IllegalStateException` instead. The inline branch is already completed, so the marking is inert there and a wait on it still returns at once.
...
- `public String applyHere(@Nullable Player viewer, String text)` - (1.23.0) resolves PAPI tokens on the CALLING thread, main or not. `apply` is the safe default and deliberately skips off the primary thread; this is the documented escape hatch for the one shape that cannot use it - text built ON an event thread that must already carry resolved placeholders when it is sent. `AsyncChatEvent` is the case it exists for: a chat format, its hover and its click are assembled off-thread and handed to a renderer Paper also invokes off-thread, so there is no later main-thread point, and blocking a hop back would cost a tick per message on a chat plugin's hottest path. What the caller accepts: PAPI expansions are third-party code with no thread-safety contract - the ones reading cached player state (effectively all a chat format uses) are fine, one touching the world is not. Fail-open: an expansion that throws leaves the ORIGINAL text and logs one debug line. Same fast paths as `apply` (null text, no `'%'`, PAPI absent). Prefer `apply` everywhere else and `applyOnMain` when the result is consumed through `thenSync`; reach for this only on an event thread.
- `public List<String> applyHere(@Nullable Player viewer, List<String> lines)` - (1.23.0) list overload, resolving line by line; a null or empty list returns the same reference.
...
- `boolean registerExpansion(String identifier, String author, String version, Map<String, Function<OfflinePlayer, String>> exact, Map<String, BiFunction<OfflinePlayer, String, String>> prefixed, Set<String> globalExact, Set<String> globalPrefixed)` - (package-private) the builder's bridge to `PapiHolder.register`. The two sets name the keys whose resolver accepts a null requester (1.25.0).

### ExpansionBuilder
`src/main/java/com/sn/lib/papi/ExpansionBuilder.java`

Declarative builder of a PlaceholderAPI expansion, obtained via `SnPapi#expansion(String)`. The built expansion reports `persist() = true` (it survives PlaceholderAPI expansion reloads and only the context teardown removes it) and null-checks the requesting `OfflinePlayer` before touching a PLAYER-BOUND resolver: a null player leaves those tokens unresolved. Cache-only contract: resolvers run on the main thread inside PAPI's parse, so they must read precomputed in-memory state and never touch disk, database or network.

Global placeholders (1.25.0): not every placeholder describes a player. A leaderboard row, a server-wide counter or an event countdown is global data exposed through a per-player API, and the callers that ask for it supply no player - a global hologram, a Discord bridge, a console task, `/papi parse --null`. `global(...)` and `globalPrefixed(...)` bind a resolver that takes NO `OfflinePlayer` (so it cannot dereference one) and therefore answers a null requester. `placeholder(...)` / `prefixed(...)` keep the short-circuit, so a resolver written against the non-null contract stays safe.
...
- `public ExpansionBuilder placeholder(String param, Function<OfflinePlayer, String> resolver)` - binds `%<identifier>_<param>%` to the resolver. Case-insensitive matching (the key lowercases with `Locale.ROOT`); exact placeholders WIN over prefixed ones. The resolver never sees a null player.
- `public ExpansionBuilder prefixed(String prefix, BiFunction<OfflinePlayer, String, String> resolver)` - binds every `%<identifier>_<prefix><rest>%` to the resolver, which receives the rest after the prefix as its second argument. Prefixes try IN REGISTRATION ORDER (LinkedHashMap), after the exacts. The resolver never sees a null player.
- `public ExpansionBuilder global(String param, Supplier<String> resolver)` - (1.25.0) binds `%<identifier>_<param>%` to a resolver that takes NO player, so it answers a null requester too. Same keyspace and precedence as `placeholder`: the later of the two declarations for one param wins outright (declaring it re-adds the key to the global set, declaring it as `placeholder` removes it).
- `public ExpansionBuilder globalPrefixed(String prefix, Function<String, String> resolver)` - (1.25.0) prefixed counterpart of `global`: the resolver receives only the rest after the prefix and answers a null requester. Same keyspace as `prefixed`; the later declaration for one prefix wins.
...
- PAPI resolution is main-thread only by default: off the primary thread `apply` leaves the tokens intact (with a debug note). Two bridges exist for async flows and they answer different questions. `SnPapi.applyOnMain` (v1.1) hops the resolution to the main thread and returns an `SnFuture` - correct when the result is consumed later, through `thenSync`. `SnPapi.applyHere` (1.23.0) resolves on the calling thread - correct when there IS no later main-thread point, above all text assembled inside `AsyncChatEvent` and handed to a renderer Paper invokes off-thread. Never `join()` an `applyOnMain` future from an event thread to fake the second case: that blocks the event thread for a full tick per call. Waiting on one from the MAIN thread is worse and, since 1.27.0, no longer silent - the off-main future is `wrapMainCompleted`, so it throws instead of deadlocking the server on a hop only that same thread could have run.
...
#### Canonical enumeration of the 18 listeners (with their origin package)
...
13. `SelfUpdater.joinListener()` - `com.sn.lib.update.internal` (SnLib self-update notice on join, v1.16).
14. `new HologramChunkListener()` - `com.sn.lib.hologram.internal` (chunk load/unload for holograms).
15. `new SelectionWandListener()` - `com.sn.lib.region.internal` (cuboid selection wand, v1.1).
16. `new TeleportMoveListener()` - `com.sn.lib.teleport.internal` (warmup teleport cancel on move, v1.6).
17. `new TeleportDamageListener()` - `com.sn.lib.teleport.internal` (warmup teleport cancel on damage, v1.6).
18. `new CommandPriorityListener()` - `com.sn.lib.command.internal` (takes back the keys of the roots with command priority, v1.39.0; section 13).
...
- `public SnItem flags(List<String> names)` - Adds ItemFlags by name. Two names are SnLib's own, resolved in `applyFlags` before `ItemFlag.valueOf` is ever reached: `HIDE_ALL` expands to this server's `ItemFlag.values()`, and `HIDE_TOOLTIP` (1.29.0) hides the WHOLE tooltip box, display name included, via the `hide_tooltip` component (`ItemMeta.setHideTooltip`, 1.20.5+, through `SnCompat.probe`; on 1.20.4 ONE WARN and the tooltip stays). `HIDE_TOOLTIP` is deliberately NOT part of `HIDE_ALL` - folding it in would erase the display name of every item already asking for `HIDE_ALL`. An unknown name tries the `HIDE_POTION_EFFECTS`/`HIDE_ADDITIONAL_TOOLTIP` alias (bidirectional) before ONE WARN.
...
- `public SnItem itemModel(String key)` - Base ItemModel key (1.21.2+ `minecraft:item_model` component), e.g. `nexo:2d_player_head`; no namespace defaults to `minecraft:`. Independent of `modelData` - both components can coexist on one item. Applied via `SnCompat.probe(ItemMeta, "setItemModel")`: below 1.21.2 or with an invalid key it is skipped with ONE WARN. Null or blank is a no-op.
...
### Multi-line lore (1.12.0)

`SnItem.lore(List)` splits any line containing `\n` into one lore line per segment (trailing newlines add no empty line; each segment carries its own colour codes). Because both `SnItem.fromConfig` and the GUI template render path funnel every lore line through this method AFTER placeholder resolution, a multi-line Ph value (`String.join("\n", lines)`) bound against a single `{placeholder}` template line expands into real lore lines - the supported way to feed a config LIST (a reward description, a rules block) into one template placeholder.

### Redeemables (1.12.0)

`src/main/java/com/sn/lib/item/RedeemSpec.java`, `RedeemHandler.java`, plus the registry surface `ItemRegistry.redeemable(id, spec, handler)` / `removeRedeemable(id)` / `redeemEntry(id)` (nested `ItemRegistry.RedeemEntry(spec, handler)` record) and the dispatch inside `ItemInteractListener` (flow step 2.5).

- Registration is programmatic and per-context: entries live in the `ItemRegistry`, so they survive item reloads (like programmatic defs) and die with the owner on disable via the tenant sweeper. Re-registering an id replaces the entry; a reload callback re-registering with fresh config is the intended pattern for config-driven modes.
- Dispatch: any right-click (air or block, sneaking or not, from either hand) whose hand item resolves to a registered redeemable. Runs AFTER the item cooldown and interact-requirements, and REPLACES variant dispatch and durability for that click. `RIGHT_CLICK_AIR` works because the shared listener never sets `ignoreCancelled` (Bukkit fires air interactions pre-cancelled: null clicked block -> `useInteractedBlock` DENY).
- Safety: a use already denied by another plugin (`useItemInHand()` DENY) cuts the flow without consuming; otherwise BOTH event results are denied before consuming, so a placeable material (player head) never places and a clicked container never opens. A click on a `RedeemSpec.blockedOn(materials)` block skips redemption entirely so the block interaction wins.
- Consumption (`RedeemSpec.Mode`): `SINGLE` (one unit from the used hand), `HAND_STACK` (the whole held stack), `ALL_MATCHING` (every matching stack of storage+armor+offhand+cursor, mirroring `ItemRegistry.take`, with an optional unit cap `allMatching(limit)`; 0 = uncapped). One physical click redeems at most once: a per-player same-tick stamp absorbs the second event of a dual fire (same redeemable in both hands), so the handler never runs twice nor doubles a capped sweep; a redeem that removes 0 units never invokes the handler. The stamp entry drops on player quit.
- `RedeemHandler.redeem(player, amount, consumed)`: main thread, `amount` always positive, `consumed` an immutable snapshot of the removed chunks (clones sized to what each slot lost) so per-stack PDC data (value-carrying currency notes) can be aggregated.
...
- 1.20.4 compat: `glow()` degrades to a real enchantment (LURE) plus HIDE_ENCHANTS, and `max-stack-size` and the `HIDE_TOOLTIP` flag are omitted; all with ONE WARN via the SnCompat probe.
...
Since 1.28.0 a menu can also RECEIVE an item, strictly additively: the item-level `input: true` marks a cell as an input slot, the menu-level `player-inventory: open` leaves the viewer's own inventory usable, and a stack aimed at the menu is reported to the session as an `ItemOffer` through `GuiSession.onOffer`. The event behind every offer is CANCELLED before the handler runs and the offer carries a clone, so the library reads the item and never moves it - accepting one, and writing back the remainder of a partial deposit, is the consumer's code. A menu that declares neither key routes byte-identically to 1.27.0 (the routing table lives in the pure `gui/internal/OfferRouting`, exhaustively unit-tested against exactly that property).
...
Per-click matrix (1.1.0): besides the generic `click-actions`/`click-requirements`/`deny-actions`, six click keys (`right`, `left`, `shift-right`, `shift-left`, `middle` and, since 1.31.0, `drop`) each read three optional lists (`*-click-actions`, `*-click-requirements`, `*-click-deny-actions`; 18 keys total, a 6x3 matrix stored in an immutable `EnumMap` of the package-private `ClickKey` enum with the private record `PerClick(actions, requirement, denyActions)`). A list counts as declared only if non-empty; a specific requirement parses ONCE and only if its list is non-empty (an absent one stays null so it can fall to the generic). Resolution is field-by-field and specific-over-generic: the click's exact shift entry wins, then the side entry (RIGHT groups RIGHT/SHIFT_RIGHT; LEFT groups LEFT/SHIFT_LEFT/DOUBLE_CLICK/CREATIVE, consistent with `ClickType.isLeftClick()`; MIDDLE goes alone), then the drop entry (1.31.0: DROP groups DROP and CONTROL_DROP, the way a side groups its shift twin), and finally the generic field. Each field falls to the generic independently: an item can declare `right-click-actions` without `right-click-requirements` and its requirement still resolves from the generic `click-requirements`. The three specific tiers are disjoint by construction - a click that has a shift or a side key never has a drop key - so the drop entry only ever competes with the generic fallback, which is why a menu with no `drop-click-*` declared resolves exactly as it did before 1.31.0. Note the deliberate asymmetry with the ACTION guards of section 08: the matrix key `drop` groups both drop clicks, while the guard `[drop-click]` stays exact (only DROP, never CONTROL_DROP) - guards match a ClickType, matrix keys group a gesture.

Package-private enum `NavKind` (navigation role, detected from the action lists when parsing): `NONE`, `PREVIOUS`, `NEXT`.

- `static @Nullable GuiItemDef parse(SnYml yml, String path, String id, @Nullable Map<Character, int[]> layout, Consumer<String> warn)` (package-private) - Parses the item at `path`; warnings go to `warn`. Returns null when the section does not exist ("Section '<path>' does not exist in <file>; item ignored"). Parses slots via `SlotParser`, `update-interval` (clamped to >= 0), `view-requirements`/`click-requirements` via `RequirementEngine`, `click-actions`/`deny-actions` and the per-click matrix (one entry per click key that declared at least one of its three lists); it detects `NavKind` by looking for `[previous-page]`/`[next-page]` in ALL the action lists (the generic one and the six per-click entries) and, if it is a navigation item and the `nav-disabled` subsection exists, parses it recursively as an override (with a null `layout`: key does not apply there). The `layout` parameter (1.1.0) is the character -> slots map of the menu's `layout:` (an empty map for a layoutless menu, null only for nav-disabled overrides; since 1.18.0 templates receive the map too) and resolves the `key:` field as an alternative to `slots:`: an empty key changes nothing; a null `layout` WARNs "Item '<id>': 'key' does not apply in this section; ignored" and the item parses normally; with slots already declared it WARNs "Item '<id>': declares 'slots' and 'key'; slots wins and key is ignored"; a key whose trim is not exactly 1 character WARNs "Item '<id>': key 'RAW' invalid (must be 1 character); key ignored"; a layoutless menu WARNs "Item '<id>': 'key' declared but the menu has no layout; ignored"; a character absent from a NON-empty map is SILENT since 1.19.1 - that is the owner hiding the button by removing its letter, the supported way to remove one, and it sets the package-private `hiddenByLayout()` flag so `GuiDef.parse` skips its own warning too (before 1.19.1 the pair cost two WARNs per hidden button on every boot, for configuration that was working as intended); a present character assigns `slots = layout.get(c).clone()`, so a key appearing in N cells renders in all N (replacing other libs' fillKey/fillEmptyKey without extra API). Since 1.18.0 an invalid key no longer returns null: it WARNs and leaves the slots empty, so ITEMS are then dropped by `GuiDef.parse`'s `hasSlots()` gate (with its "no valid slots" WARN) while TEMPLATES - whose declared placement is optional - stay usable through explicit slot binds.
...
- `boolean specificActionsFor(@Nullable ClickType click)` (package-private) - True when the click resolves its actions from a declared specific list (shift, side or - since 1.31.0 - drop entry) instead of the generic fallback; consumed by `GuiSession.runClick`'s strict-clicks gate. This method IS the gate's whole notion of "the item covers this click", which is why the drop key entered strict mode by teaching it about the drop entry instead of carving an exception into the gate itself.
- `static @Nullable ClickKey shiftKey(@Nullable ClickType click)` / `static @Nullable ClickKey sideKey(@Nullable ClickType click)` / `static @Nullable ClickKey dropKey(@Nullable ClickType click)` (1.31.0) / `static boolean basicClick(@Nullable ClickType click)` (package-private) - Pure resolution mappings: the exact shift key (only SHIFT_RIGHT/SHIFT_LEFT), the side key (RIGHT groups RIGHT/SHIFT_RIGHT; LEFT groups LEFT/SHIFT_LEFT/DOUBLE_CLICK/CREATIVE; MIDDLE alone; keyboard and unknown clicks, NUMBER_KEY/DROP/CONTROL_DROP/SWAP_OFFHAND/UNKNOWN, have no side and return null - DROP and CONTROL_DROP never became a side, they got their own mapping), the drop key (DROP and CONTROL_DROP alike, null for every other click, so NUMBER_KEY/SWAP_OFFHAND/UNKNOWN still map to no entry at all) and the predicate of the 4 basic mouse clicks (LEFT/RIGHT/SHIFT_LEFT/SHIFT_RIGHT). Covered by `ClickResolutionTest`.
- `public String id()` - The item's id (its key inside `items:` or `templates:`).
- `public int[] slots()` - Slots where it renders (defensive copy via `clone()`); for templates these are the yml-declared cells (`slots:` or `key:`, 1.18.0) consumed by the no-slot `GuiSession.bind(String, Ph...)`, empty when the template declares neither.
...
- `public boolean input()` (1.28.0) - Whether this cell RECEIVES an item (`input: true`, read with `sec.getBoolean("input", false)`, so absent means false and every pre-1.28.0 file parses identically). The cells it resolves to (`slots:` or its layout `key:`) become input slots of the menu, collected by `GuiDef.parse`; templates carry the flag too and contribute the slot they are BOUND to, not the one they declare. A click on such a cell with a stack on the cursor, or a drag over it, is reported as an `ItemOffer` instead of firing the cell's click actions - but only when the cursor is non-empty, so an input cell clicked empty-handed is still an ordinary button. `nav-disabled` override sections parse the key like any other and it means nothing there (a disabled arrow fires nothing at all).
...
- `ItemStack renderOver(ItemStack supplied, @Nullable Player viewer, Ph... phs)` (package-private, 1.21.0) - Renders a PLUGIN-SUPPLIED stack under this definition instead of building one from the yml: a clone of `supplied` carries the whole appearance and only this section's `display-name` (replacing) and `lore` (appended) are painted over it, both read per viewer (`yml.getString`/`getStringList` with the viewer, so PAPI resolves) plus the locals `phs`. The overlay rules themselves live in `StackOverlay`. There is NO skin-refresh hook on this path by design: the appearance is the caller's stack, so there is no `skull-owner` left to resolve and firing a fetch would be pointless work.

### StackOverlay (package-private, 1.21.0)
`src/main/java/com/sn/lib/gui/StackOverlay.java`

The appearance overlay a template paints over a plugin-supplied stack, split into pure rules plus the one Bukkit step so the contract is unit-testable without a server (`StackOverlayTest`). The dividing line it enforces: the STACK is authoritative about how the cell looks, the TEMPLATE about how it behaves, and exactly two appearance fields cross that line because both are text a server owner must be able to write. Everything else of the template (material, amount, glow, enchantments, custom-model-data, ...) is deliberately NOT applied - the whole reason the surface exists is that a stack the plugin did not author (crate reward, kit contents, shop stock, lootbox preview) carries NBT no yml item definition can re-express.

- `static @Nullable Component name(@Nullable String declared, Ph... phs)` - The name the template contributes, or null meaning "leave the stack's own name alone" (never "clear it"). Emptiness is judged AFTER `SnText.applyLocals`, exactly like `SnItem.fromConfig`, so a `display-name` that is one placeholder the entry left empty overrides nothing. Rendered through `SnText.color(...)` + `decorationIfAbsent(ITALIC, FALSE)`, the same two steps `SnItem` applies.
- `static @Nullable List<Component> lore(@Nullable List<Component> existing, @Nullable List<String> declared, Ph... phs)` - The stack's existing lore followed by the template's declared lines; null when the template declares none, which is the signal to leave the lore untouched (an empty list would clear it). Each declared line resolves its locals and splits on `\n` into one line per segment, mirroring `SnItem.lore(List)`, so a multi-line placeholder value behaves the same on both render paths. A declared BLANK line survives as a spacer; only an empty LIST counts as "declared nothing".
- `static ItemStack apply(ItemStack supplied, @Nullable String declaredName, @Nullable List<String> declaredLore, Ph... phs)` - Paints the overlay over a CLONE of `supplied` and returns the clone, so the caller's instance is never mutated and never reaches the inventory. A stack with no meta (AIR) returns as the plain clone.

**Strict pass-through (the guarantee consumers build on).** A template that declares NEITHER field never fetches the meta and never writes it: the return value is the plain clone, equal to the input field for field, with nothing appended, nothing cleared and nothing normalized. That is what a consumer needs when it hands over a fully-formed stack whose lore it built itself and wants the menu to add exactly nothing. Every shape of "undeclared" reaches it: an absent key (the `""` / `List.of()` defaults `renderOver` reads), a blank `display-name: ""`, and an empty `lore: []`. The one distinction the strictness rests on is empty LIST versus list-holding-an-empty-STRING: `lore: []` is "declared nothing" (leave the stack's lore alone), while a list with a blank entry is "declared one blank line" (a deliberate spacer, appended like any other). The meta is fetched ONLY once something is actually going to be written to it, so the guarantee is structural rather than a matter of writing back the same values. Covered by `StackOverlayTest.everyShapeOfAnUndeclaredFieldIsAStrictPassThrough` and `anUndeclaredLoreIsNeverConfusedWithADeclaredEmptyOne`.

**Scope of that guarantee.** It is the contract of the OVERLAY. The stack the session finally places in the inventory additionally carries the `snlib_gui_item` anti-theft marker - one PDC key written by `GuiSession.stamp` after `apply` returns, exactly as on every rendered GUI stack since 1.0.0. So "untouched" means "the overlay added nothing"; it does not mean the inventory stack is PDC-identical to what the plugin passed in, and it never could be without giving up the anti-theft protection.

**Allocation cost, per bind and per render** (stated so a consumer can size the surface for their own use case instead of guessing):
- Per `bind(slot, template, stack, phs)` CALL: one `Ph[]` clone and one `Binding` record (both pre-existing on the 3-argument form) plus one `ItemStack.clone()` of the supplied stack. The paged and region paths add nothing per bind - their `PhCollector` / `GuiEntry` instance already existed.
- Per RENDER of a supplied cell: one `ItemStack.clone()`, always. Then, only for what the template actually declares - a `display-name` costs one resolved String and one `Component`; a `lore` costs one pre-sized `ArrayList` plus one `Component` per final line. A BARE template costs the clone and nothing else: no meta fetch, no meta write. On top of that, unchanged from every other render path, `stamp` fetches the meta, writes one PDC key and writes it back.
- For comparison, the template path this replaces runs `SnItem.fromConfig` (about twenty yml reads), builds an `SnItem` with its internal lists, allocates a fresh `ItemStack` and writes the full meta. A supplied stack is therefore strictly CHEAPER per render than the template render it stands in for.
- Sizing: this is a per-viewer declarative session, built for opens, refreshes, page changes and `update-interval:` ticks. It is deliberately NOT tuned for a per-frame full-strip repaint at animation rates (10 Hz and up) - that belongs on a raw Bukkit inventory, and the one record per bind is not worth contorting the design to avoid.
...
- `static GuiDef parse(Sn ctx, String id, SnYml yml)` (package-private) - Parses the whole file; every malformed field emits a WARN prefixed `[gui <id>]` and falls back, never throwing. Empty/unreadable file: WARN "Empty or unreadable file; using a default gui with no items" and returns a default GUI (title "Menu", 3 rows, no items, empty `pagedSlots`). `rows` outside 1-6 WARNs and uses 3 (a check that only applies WITHOUT a layout). Items without valid slots WARN ("Item '<key>' has no valid slots; not rendered") and are discarded, EXCEPT when `hiddenByLayout()` is set (1.19.1): an item whose `key:` letter the owner removed from the layout is discarded silently, since hiding a button that way is the documented removal path. Templates parse without requiring slots. Both loops pass the layout's key map to `GuiItemDef.parse` (an empty map when there is no layout, never null): since 1.18.0 templates resolve `slots:`/`key:` exactly like items - the declared cells feed the no-slot `GuiSession.bind(String, Ph...)` - and a template declaring neither stays a bind-anywhere template.
...
- `regions:` (1.20.0, `parseRegions` + `parseRegionCells` + `noteRegionCollisions`): a menu-level section of NAMED groups of cells, the destination of `GuiSession.bindEach`. A scalar value is shorthand for `key:`; the long form reads `slots:` (which wins, same precedence and wording as an item) or `key:`. Malformed declarations WARN and yield ZERO cells but the region stays DECLARED - that distinction is the whole point: a declared-but-empty region is the owner turning it off (silent), while an id nobody declared is a plugin bug (`bindEach` WARNs once). WARN cases: both slots and key ("declares both 'slots' and 'key'; slots wins and key is ignored"), a section that declares OTHER keys but neither real one ("declares neither 'slots' nor 'key'; region has no cells"), a key whose trim is not 1 character, a key without any layout, and `regions:` that is not a section. SILENT cases, both mirroring `GuiItemDef` at 1.19.1 - there are TWO supported ways to turn a region off and neither may cost a boot warning: a valid single-character key the non-empty layout does not contain, and an explicitly blanked declaration (empty value, empty section, or a declared `slots:` that parsed to nothing). The `emptiedOnPurpose` flag is what separates "the owner wrote an emptiness" from "the entry is malformed". Cells overlapping the `paged-key` or an earlier region WARN once per region naming the FIRST colliding slot only (the paged bind wins there; between two regions the later declaration wins). The map is insertion-ordered (`Collections.unmodifiableMap(new LinkedHashMap<>(...))`, never `Map.copyOf`, which loses order) because `regionIds()` drives the deterministic render order.
- `public int[] regionSlots(String regionId)` (1.20.0) - Cells the menu declares for a region, in declaration order (ascending row-major for a `key:`, first-seen order for a `slots:` list), as a defensive `clone()`. Empty BOTH for an undeclared region and for one the owner emptied - the public accessor deliberately does not distinguish them. It exists so a plugin that must not truncate silently can read the owner's cell count and say so in its own lang file, since SnLib itself never warns about a region sized down.
- `@Nullable int[] region(String regionId)` / `Set<String> regionIds()` (package-private) - The null-vs-empty distinction `bindEach` needs, and the file-order id set the render pass walks.
- `static Map<Character, int[]> layoutKeys(List<String> rows)` (package-private since 1.20.0, was private) - Package-private only so `GuiRegionTest` can drive `parseRegions` over a real mask without a server.
...
- `player-inventory:` (1.28.0, `PlayerInventoryPolicy.resolve`) - Menu-level policy over the viewer's OWN inventory, resolved leniently exactly like `inventory-type` (blank = the `LOCKED` default, case-insensitive, `-` reads as `_`, an unknown value WARNs "Invalid player-inventory '<raw>'; using LOCKED" and falls back). It governs the BOTTOM inventory only; clicks over the menu's own cells are cancelled under either policy.
- `public PlayerInventoryPolicy playerInventory()` (1.28.0) - The resolved policy, read by the shared click listener through `GuiSession.playerInventory()`.
- Input slots (1.28.0): `parse` collects the slots of every DECLARED item with `input: true` into an immutable `Set<Integer>`, exposed to the session through the package-private `boolean inputSlot(int slot)`. Templates contribute no slot here (their cell comes from the bind, so `GuiSession` registers it at bind time), only their flag. `private static void noteUnreachableInputs(...)` WARNs once per menu when any item or template declares `input: true` while the policy is `LOCKED`: "input: true declared with player-inventory: locked; the viewer can never pick a stack up, so nothing can be offered - set player-inventory: open". That combination is dead configuration rather than a broken file (the cell parses and renders normally), and the WARN is the twin of the paged-key/pagination one - the mistake is real but the file still loads.
...
### PlayerInventoryPolicy (1.28.0)
`src/main/java/com/sn/lib/gui/PlayerInventoryPolicy.java`

What a menu lets its viewer do with their OWN inventory while it is open, declared per menu through `player-inventory:` and defaulting to `LOCKED` (the behaviour of every release before 1.28.0). Two constants:

- `LOCKED` (default) - Every click and drag over the player's own inventory is cancelled while the menu is open. Nothing moves, nothing can be picked up, and the viewer's cursor stays empty for the whole session.
- `OPEN` - The viewer uses their own inventory normally: plain clicks, number keys, drops, offhand swaps and drags that stay inside the player inventory are left UNCANCELLED. Two things stay cancelled regardless and are what makes that safe: every click over the menu's own cells, and `COLLECT_TO_CURSOR`. A shift-click on a bottom stack becomes an OFFER instead of a move, because a shift-click aims INTO the menu and SnLib never writes into a menu's inventory on a consumer's behalf.

- `static PlayerInventoryPolicy resolve(@Nullable String raw, Consumer<String> warn)` (package-private) - The lenient resolution described under `GuiDef`. Pure and Bukkit-free, covered by `PlayerInventoryPolicyTest`.

**The two keys are orthogonal.** The policy governs the BOTTOM inventory, `input:` governs individual TOP cells. A menu may open the player inventory without declaring one input cell (the viewer just moves their own items while the menu is up), and an input cell is read the same way under either policy. In practice an input cell needs `OPEN` to be REACHABLE - under `LOCKED` the viewer can never pick a stack up, so their cursor is always empty and no offer can be produced - which is exactly what the parse WARN says.
...
Extra menu folders (1.36.0): a modular consumer keeps each module's menus next to the rest of that module's files through `loadFolder(folder, namespace)`, e.g. `loadFolder("modules/party/guis", "party")`. Those menus load under `"<namespace>:<file name>"` ids (`party:shop`); a `guis/` id has no `:` unless its file is literally named that way (Windows forbids it, Linux allows it), and that edge case is caught by the collision WARN. Each folder owns its namespace. `[open] party:shop`, the reload flow's reopen and the anti-theft tag (payload `<guiId>:<slot>`, only the key's presence is checked) all accept the namespaced id unchanged. Registered folders are remembered (`folders`, a `LinkedHashMap` guarded by the `guis` lock) and reloaded together with `guis/`.
...
- `public void load()` - Seeds the bundled `guis/*.yml` (via `seedBundledGuis`), creates `guis/` if still missing and (re)parses one GUI per `.yml` file (alphabetical order by name), skipping the merge backups (1.39.0, see `isMenuFile`). Requires the yml module: without it, WARN "guis() declared without config(): the guis/ folder cannot load and sn.guis() stays empty" and returns. If the folder cannot be created: WARN "Could not create the folder <path>". After `guis/`, every folder registered through `loadFolder` is seeded and parsed again (1.36.0). If `guis()` is declared but NO menu loaded at all, one WARN "guis() is declared but no menu was loaded: the guis/ folder and every folder registered through loadFolder are empty. Bundle the menus as guis/*.yml (or <folder>/*.yml) in the jar so they seed, or drop the files into the folder." (1.5.0; since 1.36.0 the check runs ONE TICK LATER through `ctx.scheduler().syncLater(1L, ...)`, because the context loads `guis/` before `onInnerEnable`, where a consumer registers its folders, and an immediate check would warn falsely; `emptyCheckPending` keeps it to one scheduled check, and a `load()` while the plugin is disabled warns immediately instead of scheduling). A `guis/` folder that cannot be created only skips `guis/`: the registered folders still load. Synchronous I/O by design: it runs only in onEnable and the reload flow (so seeding also runs on every reload - missing files reseed, existing files re-merge). File mounts cache in a `ConcurrentHashMap` (`mounts`), so the reload can re-read from disk.
- `public void loadFolder(String folder, String namespace)` (1.36.0) - Registers an extra menu folder next to `guis/`: seeds the jar's bundled top-level `<folder>/*.yml` (managed, gated by `update-configs`, exactly like `guis/`, via `GuiSeeder.seed(jar, dataFolder, folder, gate, logger)`), creates the folder if missing and loads one menu per file under `"<namespace>:<file name>"`. The folder is remembered, so every reload reloads it with `guis/` and picks up new files. Registering the same folder again closes its open menus, re-reads its mounted files from disk after seeding and reloads it under the given namespace. A namespace already used by ANOTHER registered folder throws `IllegalArgumentException`, so removing one folder's menus never touches another's. An id already loaded (a `guis/` file literally named `party:shop.yml`) is skipped with one WARN "Menu id '<id>' from <folder> is already loaded; the file is skipped" (first wins). Requires the yml module, like `load()`. Folder: `\` normalized to `/`, empty and `.` segments dropped, `..` resolved; blank, holding `:`, climbing out of the data folder or resolving to `guis` rejected (shared `yml.internal.ResourceFolders`); namespace: non-blank, no `:` and no whitespace; all throw `IllegalArgumentException`.
- `public void unloadFolder(String folder)` (1.36.0) - Forgets a registered folder: its menus stop resolving through `get` and every open session of a menu in its namespace is closed. An unknown folder is ignored; the files stay on disk and their yml mounts stay cached (a later `loadFolder` of the same folder re-reads them from disk before parsing).
...
- `private boolean isMenuFile(String dirPath, String name)` (1.39.0) - the filter of both listers (`guis/` and every `loadFolder` folder): a `.yml` (case-insensitive) that is NOT a merge backup `old-<file>-<yyyyMMdd-HHmmss>.yml` (`yml.internal.MergeBackups.isMergeBackup`, the same stamp `YamlUpdater` prunes by). Until 1.38.0 the backups the merge of a managed menu leaves in its folder loaded as ghost menus `old-<file>-<stamp>` / `<ns>:old-<file>-<stamp>`. A skipped backup is logged through `ctx.debug().log` only; a menu merely named `old-town.yml` or `oldies.yml` still loads.
- `public @Nullable Gui get(String id)` - GUI loaded under `id` (`trim()`ed), or null: the file name without extension for a menu of `guis/`, `"<namespace>:<file name>"` for a menu of a `loadFolder` folder (1.36.0).
...
- `static List<String> guiResourcePaths(Iterable<String>, String folder)` (1.36.0) - The same filter for any resource folder (`modules/party/guis`): top-level `<folder>/<name>.yml` only; the one-argument overload delegates with `guis`.
...
- `static List<String> seed(File jar, File dataFolder, String folder, @Nullable File gateFile, Logger logger)` (1.36.0) - The same pipeline for any folder, writing under `dataFolder/<folder>/`; used by `GuiManager.loadFolder`. The four-argument overload delegates with `guis`.
...
- `public void open(Player player, Ph... phs)` (1.22.0) - Opens at page 1 with local placeholders resolved into the declared `title:`, for a menu whose SUBJECT is not its viewer: another player's inventory, the clan being inspected, the crate being previewed. PAPI already covers "your" menu because it resolves against the viewer; this covers the other case. The locals are seeded BEFORE the first render, so the window carries the resolved title from the moment it appears - unlike `GuiSession.titlePlaceholders`, which has to recreate the inventory. Overload resolution note: this is strictly additive. `open(p)` and `open(p, 2)` still bind to the exact-arity methods above, because varargs are only considered in the third resolution phase - verified at the bytecode level, so no existing call site changes target or pays an array allocation.
- `public void open(Player player, int page, Ph... phs)` (1.22.0) - The same on a given page.
...
- `public @Nullable GuiItemDef itemAt(int slot)` - The definition rendered at `slot` for this viewer with precedence: API bind, then paginated entry, then the slot's declared candidates in declaration order (since 1.17.0 several items may declare the same slot; the first whose view requirement passes for this viewer is the one returned). Null for an empty slot, which since 1.16.1 also covers a definition hidden from this viewer by its `view-requirements` and a paginated slot the current page left empty (before, both returned a definition the viewer could not see; the paginated case even fell through to the declared item underneath). Resolved by the private `renderedAt(int)`, the same helper `handleClick` uses, so the query and the click can never disagree.
- `public void bind(String templateId, Ph... phs)` (1.18.0) - Binds a template into the cells IT declares in the yml (`slots:` or `key:` resolved against the menu `layout:`, declared slots win over key - same rules as items), so the server owner repositions the element by editing the file instead of the placement living hardcoded in Java. A key covering N cells renders the same bind (same placeholders) into every cell. Same lifetime and precedence as the explicit `bind(int, ...)`. An unknown template id WARNs once ("bind on gui '<id>' ignored: template '<tid>' does not exist"); a template that declares neither `slots:` nor a valid `key:` WARNs once ("declares neither 'slots' nor a valid 'key'; declare one in the yml or use bind(slot, template)") and is ignored. This is the ONE-element form (every cell of the key shows the same thing); a group of cells needing one distinct entry each is a region - see `bindEach`.
- `public <T> void bindEach(String regionId, List<T> data, BiConsumer<T, GuiEntry> filler)` (1.20.0) - Lays an immutable snapshot of `data` over the cells the menu declares for `regionId` under `regions:`, ONE ENTRY PER CELL in declaration order, the filler running once per entry on EVERY render to pick that entry's template (`GuiEntry.template`) and fill its locals (`GuiEntry.add`). This is what removes the hardcoded `int[]` from consumer code: cardinality, order, position and existence of the region all live in the yml. Cardinality belongs to the owner and NEVER warns (1.19.1 rule) - overflow drops the tail, underflow leaves the spare cells to the declared items, both recorded as ONE `ctx.debug()` note per bind, never per render. Ownership is per CELL, tracked in `regionCells` (slot -> the same private `Binding` record manual binds use): a cell with no entry, an entry whose filler picked no template and an entry hidden by its own `view-requirements` are ABSENT from the map and fall through to the declared item, on the screen (`renderRegionCell` returns without writing) and on the click (`renderedAt` falls through to `declaredAt`) alike. An undeclared region id WARNs once ("the menu declares no region '<id>' under 'regions:'; to REMOVE a region keep the declaration and take its letter out of the layout"); a filler picking an unknown template id WARNs once per gui+region+template. Null `data` clears the region; a null filler throws (`Objects.requireNonNull`, exactly like `bindPaged`). No `pagination:` needed and the page is never read nor mutated. Precedence on a shared cell: manual bind > paged bind > region > declared item. Re-binding replaces the whole region atomically (`regionBinds` is a volatile immutable map, copy-and-swap) and repaints, so a shorter list releases its tail on the same render - the stale-ghost hole `binds` cannot close. The skin-refetch guard is the bind IDENTITY (`regionBinds.get(regionId) != bind`), never the index, so a rebind between request and landing cannot repaint a vanished entry.
- `public void bind(int slot, GuiTemplate template, Ph... phs)` - Binds a template to a slot of THIS session with the given local placeholders and renders it immediately. The bind survives page refreshes and inventory recreations until overwritten; it takes precedence over an item declared on the same slot. A null template or negative slot: no-op. Always uses the given slot: the template's yml-declared cells (1.18.0) are only consumed by the no-slot `bind(String, Ph...)` overload, so plugin-computed placements (one template bound N times with different data) keep working. Since 1.21.0 it delegates to the 4-argument overload with a null stack, so there is ONE implementation and its behaviour is unchanged byte for byte.
- `public void bind(int slot, GuiTemplate template, @Nullable ItemStack stack, Ph... phs)` (1.21.0) - Same bind with the APPEARANCE supplied by the plugin: `stack` renders into the slot instead of the template's own appearance and the template keeps supplying the BEHAVIOUR (`view-requirements`, the per-click matrix, click and deny actions). This is the surface for stacks the plugin did not author - a crate reward, kit contents, shop stock, a lootbox preview - whose enchantments, custom model data, head texture and custom name no yml item definition can re-express. Only two things of the template are painted over it (`GuiItemDef.renderOver` -> `StackOverlay`): a non-empty `display-name` REPLACES the stack's name and declared `lore` lines are APPENDED after the stack's own lore, both through the normal pipeline (viewer PAPI, the locals `phs`, colour, rgb); a template declaring neither is a STRICT pass-through - the meta is not even fetched, so the stack comes through with nothing added, nothing cleared and nothing normalized (see `StackOverlay` for the exact guarantee, its scope against the anti-theft marker, and the per-bind/per-render allocation cost). The stack is cloned on the way in (so the caller may keep mutating theirs) and again on every render (so the instance in the inventory is never the stored one). Everything else is identical to the 3-argument form: same precedence, same lifetime across page changes/refreshes/inventory recreations, same `snlib_gui_item` anti-theft stamp, same `itemAt`/`handleClick` resolution. A null `stack` IS the 3-argument form, so the overload is strictly additive and every call site compiled against 1.20.x keeps its behaviour. Overload resolution note: `bind(slot, tpl, null)` still binds to the `Ph...` form (a null is assignable to `Ph[]`, which wins in the non-varargs phase); write `bind(slot, tpl, (ItemStack) null, phs)` to target this one explicitly, which is what the 3-argument form itself does.
...
- `public void handleClick(int slot, ClickType click)` - Click dispatch invoked by the shared click listener with a top-inventory raw slot: it resolves what the slot actually SHOWS this viewer through `renderedAt(int)` (manual bind, paginated entry, declared candidates in declaration order, each behind its own view requirement - on a shared slot the click resolves the same winner the render drew, 1.17.0), skips disabled navigation items and delegates to `runClick`, which resolves the definition's per-click matrix (actions, requirement and deny by `ClickType`, specific-over-generic field by field) and applies the menu's opt-in strict-clicks gate, with this session as the page target and the click type in the context. No-op if the session is closed. **View requirements gate interaction since 1.16.1**: a slot that renders nothing for this viewer fires NOTHING - not the click actions, not the deny actions - so `view-requirements` never need duplicating into `click-requirements`. Until 1.16.0 the dispatch resolved the definition by slot without re-testing the view requirement, so an item hidden from a player was still clickable on its empty slot (and an empty paginated slot fell through to a declared item underneath).
- `public void onOffer(@Nullable Consumer<ItemOffer> handler)` (1.28.0) - Registers what THIS session does with an item its viewer offers it. The handler runs on the main thread, inside the inventory event, for every `ItemOffer` the menu produces (a cursor click on an input cell, a drag over one, a shift-click from a player inventory the menu opened). Last write wins, null clears, and the handler is DROPPED on teardown (it is a lambda that typically captures the whole plugin state of whatever the menu was editing; a closed session must not keep it alive). With none registered, offers are dropped with a debug note - a safe no-op, since the event was cancelled before the dispatch and the stack is already back where it came from.
- `public void handleOffer(ItemOffer offer)` (1.28.0) - Offer dispatch invoked by the shared click listener, the parallel of `handleClick`: hands the offer to the registered handler or drops it with the debug note "GUI '<id>': <KIND> offer of <MATERIAL> dropped, no offer handler registered (session.onOffer)". No-op on a closed session or a null offer. The event that produced the offer is ALREADY cancelled by the time this runs - that ordering is the read-not-consume guarantee - and nothing here writes the stack anywhere. Main-thread only. Exceptions from a consumer handler propagate to Bukkit's listener logging exactly like an action-engine failure would; the item is already safe by then.
- `public boolean isInputSlot(int slot)` (1.28.0) - Whether `slot` RECEIVES an item for this session: true when ANY definition that can occupy the cell declares `input: true` - the item declared on it in the yml (`GuiDef.inputSlot`), a template bound to it through `bind(int, GuiTemplate, ...)` (tracked per session in the `inputBinds` set), or the template a region entry painted there (`regionCells`). The yml declaration is deliberately never revoked by a bind: a plugin that paints the CURRENT contents of an input cell must not have to re-declare that the cell accepts an item. A bind does own the input state of a cell it takes from nothing: binding a plain template over an input one removes the slot from `inputBinds`. Structural on purpose - no view requirement is evaluated and no placeholder resolved - because the click listener calls it for every click over every library menu: on a menu with no input cell it is three lookups over empty collections.
- `public PlayerInventoryPolicy playerInventory()` (1.28.0) - The definition's policy over the viewer's own inventory, read by the shared click listener. Every session of a menu answers the same; the value is menu-level, never per session.
...
- `public void titlePlaceholders(Ph... phs)` (1.22.0) - Sets the local placeholders resolved into the declared `title:` for THIS session and re-renders. The declared title stays shared and immutable; only its resolution is per session. Because a title cannot be painted into an open window, a change that actually moves the RESOLVED title goes through `refreshMenu` -> `recreate`, i.e. a second `openInventory` in the same tick; when the resolved title is unchanged it is an ordinary content re-render. Passing none clears them. Prefer `Gui.open(Player, Ph...)`, which seeds the locals before the first frame and costs no recreation - this setter is for a title that has to change WHILE the menu is open. Main-thread only. Before the session has an inventory the re-render is a no-op, which is what makes the seeding path free.
...
- Next-nav debug note (1.1.0, `navUnknownNoted`; extracted to `noteNavUnknown` in 1.17.0): the FIRST time the declared render evaluates a NEXT nav with pagination on and an unknown total (`knownTotalPages() == 0`), it records via debug "GUI '<id>': next nav with unknown total pages; next never disables (use bindPaged or setTotalPages)". Once per session (main-thread field, no volatile).
...
- Shared-slot fallthrough (1.17.0, `renderDeclared(target, restrict)` + `renderCandidate` + `effectiveNow`): the declared phase is slot-driven, not item-driven. Several items may declare the same slot (same layout `key:` or overlapping `slots:`); candidates are tried in DECLARATION ORDER and the first one whose view requirement passes for this viewer owns the cell - later candidates keep only their unshared slots, and a slot every candidate hides from is cleared, never overwritten by a hidden candidate (until 1.16.2 a later hidden item nulled the slot an earlier visible item had just rendered, so DeluxeMenus-style conditional variants of one button were impossible). Requirements and prototypes still resolve ONCE per item per pass (never once per slot); the requirement tested is the effective definition's (`effectiveNow`: the `nav-disabled` override while the direction is disabled, the item otherwise). `renderItem(target, item)` (the per-item timer tick and the landed-skin re-render) delegates to `renderDeclared(target, item.slots())`, re-resolving the winner of just those slots, so a partial re-render can never let a hidden candidate overwrite what another candidate owns.
...
- Interaction resolution (1.16.1, `renderedAt(int slot)` + `shown(...)` + `declaredAt(int slot)` + `clearGhost(int slot)`): the single place that answers "what does this slot show THIS viewer", shared by `itemAt` and `handleClick`. Precedence mirrors the render phases: a manual bind first, then the paginated bind - which OWNS its slots even when the current page left them empty, so a short page never falls back to the declared item underneath - then the declared candidates via `declaredAt` (1.17.0): the slot's candidate list in declaration order, returning the first whose view requirement passes - the exact winner rule the render uses, so click and screen can never disagree on a shared slot. `declaredAt` tests the EFFECTIVE definition's requirement (the `nav-disabled` override while the direction is disabled, which is what the viewer is actually looking at; until 1.16.2 the base item's requirement was tested, so an override with its own view-requirements could be wiped by `clearGhost` on click) but always returns the declared item, whose actions the disabled-navigation gate of `handleClick` still skips. `shown` (manual binds and paginated entries) pairs the definition with its locals only while `viewRequirement().test(viewer, resolver(locals))` passes, re-evaluated at interaction time (a state change that hides the item takes effect immediately, without waiting for the next render tick); both record a debug line for a hidden definition ("GUI '<id>': slot <n> item '<id>' hidden by its view-requirements for <player>"). A click on a slot that resolves to null runs `clearGhost`, which clears the stack an older render left there IF some render phase owns the slot, so clicking a stale item makes the screen converge instead of looking unresponsive; slots the menu does not own are never touched. The `declared` flag of the resolution keeps the `nav-disabled` skip exactly where it always was: only items declared in `items:` self-disable as navigation, binds and paginated entries are unaffected.
- Clicks (`runClick`): the single funnel for declared items, manual binds and paginated entries. Strict-clicks gate FIRST: with `strict-clicks: true`, a click outside the 4 basic mouse clicks with no declared specific list covering it (`GuiItemDef.basicClick` + `specificActionsFor`) is discarded BEFORE the requirement test, with a debug note: neither the click actions nor the deny actions run (the listener already cancelled the event). `middle-click-actions` enables MIDDLE, a declared `left-click-actions` enables DOUBLE_CLICK and CREATIVE (a vanilla double click is two lefts; deliberate) and, since 1.31.0, a declared `drop-click-actions` enables DROP and CONTROL_DROP; NUMBER_KEY, SWAP_OFFHAND and UNKNOWN have no possible specific list and stay always discarded in strict, and so does a DROP over an item that declared no drop actions. With strict false (default) behavior is identical to v1.0.0. Past the gate, it builds an `ActionContext(viewer, ctx, this, click, phs)` (the surfaceless compat overload: GUI clicks have no `ClickSurface`) and runs `clickActionsFor(click)` if `clickRequirementFor(click)` passes, or `denyActionsFor(click)` if not, via `ctx.actions().run` (specific-over-generic resolution field by field).
- Plugin-supplied appearance (1.21.0): all three render paths branch on a supplied stack the same way - `renderBinding` on `binding.stack()`, `renderPagedSlot` on `collector.stack()` and `renderRegionCell` on `entry.stack()`. Non-null goes to `item.renderOver(supplied, viewer, phs)`, null takes the unchanged `item.render(viewer, skinHook(...), phs)`, and both results go through the same `stamp(...)`. The branch sits INSIDE the view-requirement test, so a hidden cell still costs no render and no clone. The skin hook is wired only on the template path: a supplied stack has no `skull-owner` to resolve, so firing a fetch (and the re-render it schedules) would be pointless. Clone accounting: the manual `bind` clones at BIND time (the stack is stored and outlives the call, so the caller may keep mutating theirs) and `renderOver` clones again per render; the two collectors do NOT clone in their setter, because a collector is created and consumed inside one render and `renderOver`'s single clone already guarantees both invariants (the caller's instance is never mutated, and never reaches the inventory) - that keeps the render path at exactly ONE allocation per supplied cell.
- Item offers (1.28.0): the session keeps `inputBinds` (a `ConcurrentHashMap` key set of the slots whose BOUND template declares `input: true`, written by the 4-argument `bind` - which also REMOVES the slot when a plain template is bound over it) and the volatile `offerHandler`. `isInputSlot` unions those with the definition's declared input slots and the live `regionCells`; `handleOffer` is the dispatch twin of `handleClick`. `teardown()` nulls the handler and clears `inputBinds` alongside the task handles. Nothing in the session ever writes an offered stack anywhere: the whole feature is a read plus a callback.
- Private records: `Binding(GuiTemplate template, Ph[] phs, @Nullable ItemStack stack)` (a manual bind with its locals captured at bind time and, since 1.21.0, the defensive copy of the plugin-supplied appearance; region cells reuse the record for the CLICK side only and always store a null stack, since a region's supplied stack is per render and retaining the caller's instance would buy nothing), `PagedBind<T>(GuiTemplate template, Pagination<T> pagination, int[] slots, BiConsumer<T, PhCollector> mapper)` (the live paginated bind) and `Rendered(GuiItemDef item, Ph[] locals, boolean declared)` (1.16.1, what a slot shows the viewer, returned by `renderedAt`).
...
A reusable GUI item declared under a GUI file's `templates:` section: the config user customizes appearance and actions freely while the plugin supplies the runtime data at bind time. Placement comes from either side (1.18.0): the template may declare `slots:` or `key:` (resolved against the menu `layout:` exactly like an item; declared slots win over key) and be bound with the no-slot `GuiSession.bind(String, Ph...)`, so the server owner repositions it by editing the file; or the plugin decides the slot in Java via `GuiSession.bind(int, GuiTemplate, Ph...)`, which ignores the declared cells. Until 1.17.0 templates could not declare placement at all (`key:` warned "does not apply", `slots:` was ignored). Since 1.20.0 there is a THIRD placement: a template painted by a runtime region takes its cell from the region's `regions:` declaration and its own `slots:`/`key:` are ignored there (only appearance, requirements and actions are used) - which is what lets several state variants of one button serve the same region, and why a template used only by a region should stay keyless. It supports exactly the same fields as a regular item (`slots:`/`key:` optional instead of required), and typically uses plugin-defined local placeholders (for example `%index%`) provided as `Ph` pairs at bind time.

- `public String id()` - Template id (its key inside the `templates:` section).
- `public int[] slots()` (1.18.0) - Cells the template declared through `slots:` or `key:`; empty when it declares neither. Target of the no-slot `GuiSession.bind(String, Ph...)`.
- `public boolean hasSlots()` (1.18.0) - True when the template declared a placement (`slots:` or a valid `key:`).
...
- `public PhCollector stack(@Nullable ItemStack stack)` (1.21.0) - Appearance of THIS paged entry, supplied by the plugin: the stack renders INSTEAD of the template's own appearance, with only the template's declared `display-name` (replacing) and `lore` (appended) painted over it. Last call wins, chainable; a null stack (or never calling it) is exactly the pre-1.21.0 behaviour. The stack is copied before anything is written to it, so the caller's instance is never mutated and never reaches the inventory. `view-requirements`, the per-click matrix, the anti-theft marker and the bind's lifetime across page changes are all unchanged.
- `public Ph[] toArray()` - Collected pairs in insertion order.
- `@Nullable ItemStack stack()` (package-private, 1.21.0) - Read-back for the paged render pass, the twin of `GuiEntry.templateId()`. The mapper runs once per entry per render, so the stack is re-read on every render and stays live under `update-interval:` instead of freezing at bind time.

### GuiEntry
`src/main/java/com/sn/lib/gui/GuiEntry.java`

One cell of a `bindEach` region while it is being filled (1.20.0): the same accumulator role `PhCollector` plays for `bindPaged`, PLUS the per-entry template choice a paged bind does not need. A fresh instance is handed to the filler per entry per render and it never exposes a slot index - the placement belongs to the yml. A DELIBERATELY SEPARATE type from `PhCollector`: adding `template()` there would either no-op silently inside `bindPaged` or change the most-used render path.

- `public GuiEntry template(String templateId)` - Template of the `templates:` section that paints this entry; last call wins, chainable. Null/empty/never called leaves the cell to the declared item (the supported way to skip ONE entry without punching a hole); an unknown id WARNs once per gui+region+template and does the same. The template's own `slots:`/`key:` are IGNORED here - the region already decided the cell, so several state variants can share one region and none of them should declare a placement. Deliberately NOT overloaded with a `GuiTemplate` variant: with both present, `entry.template(null)` would be an ambiguous-method compile error at every call site.
- `public GuiEntry add(String key, Object value)` - Same contract as `PhCollector.add`. The pairs reach the rendered item, its `view-requirements` and the `ActionContext` of its clicks, which is why an entry's identity must live here and never in its index: reordering the cells then reorders the picture and never the data.
- `public GuiEntry stack(@Nullable ItemStack stack)` (1.21.0) - Same contract as `PhCollector.stack`: the supplied stack paints this cell instead of the chosen template's appearance, under that template's `display-name`/`lore` overlay. It still needs `template(...)` - the template is what carries the click behaviour, and a cell with no definition behind it would be inert and unclickable, which is useless to the consumers this exists for. The fallthrough to the declared item, the view requirement and the anti-theft marker are unchanged.
- `String templateId()` / `@Nullable ItemStack stack()` / `Ph[] toArray()` (package-private) - Read-back for the region render pass; the filler runs per entry per render, so the stack is re-read on every render like the template id and the placeholders.

### ItemOffer (1.28.0)
`src/main/java/com/sn/lib/gui/ItemOffer.java`

The report SnLib hands a session when a viewer aimed an item of their own at the menu: `public record ItemOffer(Player viewer, Kind kind, int slot, int playerSlot, ItemStack stack, ClickType click)`, delivered to the handler registered through `GuiSession.onOffer`, on the main thread, inside the inventory event that produced it. Public nested `enum Kind { CURSOR, DRAG, SHIFT_CLICK }`.

**The guarantee: the item is READ, never consumed.** Every event that produces an offer is cancelled BEFORE the handler runs, and the record carries a defensive clone. SnLib never moves, writes, shrinks or deletes the offered stack and never writes it into the menu's inventory - the cancel is what puts the cursor, the drag or the shift-clicked stack back exactly where it was. What "accepting" an offer means belongs entirely to the consumer: read `stack()`, decide how much of it you take, write the remainder back yourself.

Which fields carry information depends on `kind()`:

| Kind | Produced by | `slot()` | `playerSlot()` | `click()` |
|---|---|---|---|---|
| `CURSOR` | LEFT or RIGHT click on an input cell with a stack on the cursor | the input cell | -1 | the click, so "right deposits one" is expressible |
| `DRAG` | a drag covering exactly ONE input cell | that cell | -1 | the mouse button behind the drag: RIGHT for `DragType.SINGLE`, LEFT for an even spread |
| `SHIFT_CLICK` | a shift-click in the player's own inventory, needs `player-inventory: open` | -1 | the player-inventory slot the stack came from | SHIFT_LEFT or SHIFT_RIGHT |

- The compact constructor requires a non-null `viewer`, `kind`, `click` and `stack`, and CLONES the stack on the way in, so an offer can never alias the live cursor, the live player-inventory slot or anything the caller keeps mutating. The accessor hands that same copy out without cloning again: it is already an instance nothing else references, and the consumer is expected to read its amount and its meta. `stack` always carries its REAL amount - a deposit flow needs it.
- `playerSlot()` is what makes a DEPOSIT expressible without SnLib ever touching an inventory: the consumer computes how much it accepted and writes the remainder back with `viewer.getInventory().setItem(offer.playerSlot(), remainder)`.
- **Ghost stacks.** The click was cancelled, so the client is still drawing the stack it had before the event. When the consumer then changes that slot server-side (the remainder of a partial deposit, or clearing it on a full one) the client does not know and keeps painting the stale stack until something else refreshes it. Every write-back must be followed by one `viewer.updateInventory()`.

### GuiClickListener (internal)
`src/main/java/com/sn/lib/gui/internal/GuiClickListener.java`

The single shared click listener, owned by SnLib, for EVERY library GUI of every consumer. Enrolled in the ListenerHub; `registerEvents` happens SOLELY in the SnLibPlugin bootstrap. Identification is ALWAYS `holder instanceof SnGuiHolder`, never by title. Since 1.28.0 it is a thin ADAPTER over the pure `OfferRouting`: it reads the event, asks for a `Decision` and applies it, with no branch of its own beyond that.

- `public void onClick(InventoryClickEvent event)` - (`@EventHandler(priority = HIGH, ignoreCancelled = true)`) Resolves the zone of the raw slot (`OfferRouting.zone`), asks `OfferRouting.click(policy, zone, action, click, isInputSlot, cursorEmpty, clickedEmpty)` and applies: `PASS_THROUGH` returns without touching the event; every other decision CANCELS FIRST and then dispatches - `CANCEL_AND_CLICK` to `session.handleClick(rawSlot, click)` (the pre-1.28.0 path), `OFFER_CURSOR` to `session.handleOffer(...)` with the cursor stack and the clicked cell, `OFFER_SHIFT` with `event.getCurrentItem()` and `event.getSlot()` (on a bottom click that is the index INSIDE the player inventory, which is the one a consumer writes a remainder back to). `isInputSlot` is only queried for a TOP slot.
- `public void onDrag(InventoryDragEvent event)` - (`@EventHandler(priority = HIGH, ignoreCancelled = true)`) Counts how many of `event.getRawSlots()` fall inside the top inventory, remembers the last one, and applies `OfferRouting.drag(policy, topSlots, isInputSlot, cursorEmpty)`: `PASS_THROUGH` leaves a drag that never left the player inventory alone (under `OPEN`), `OFFER_DRAG` cancels and reports `event.getOldCursor()` on the single covered cell, everything else cancels. A cancelled drag restores the PRE-drag cursor, which is why the read-not-consume guarantee holds for drags exactly as it does for clicks.
...
- `private static ClickType dragButton(DragType type)` - RIGHT for `DragType.SINGLE`, LEFT otherwise, so a DRAG offer carries the same button information a CURSOR offer does. By comparison, never a switch, keeping the enum open across versions.
- `private static boolean empty(@Nullable ItemStack stack)` - Null-safe emptiness (the API returns AIR or null for "nothing" alike).

### OfferRouting (internal, 1.28.0)
`src/main/java/com/sn/lib/gui/internal/OfferRouting.java`

The PURE decision core of the click listener: given what the event says, what should happen to it. Extracted so the whole routing table is exhaustively unit-testable without a server (`OfferRoutingTest`, 22 tests, most of them sweeping every `ClickType` or every `InventoryAction`). Package-private final class, private constructor, two enums and three static methods.

- `enum Zone { TOP, BOTTOM, OUTSIDE }` and `static Zone zone(int rawSlot, int topSize)` - Which inventory of the open view the event landed on; a negative raw slot (the drop-outside `-999`) is `OUTSIDE`.
- `enum Decision { PASS_THROUGH, CANCEL_ONLY, CANCEL_AND_CLICK, OFFER_CURSOR, OFFER_SHIFT, OFFER_DRAG }`.
- `static Decision click(PlayerInventoryPolicy policy, Zone zone, @Nullable InventoryAction action, @Nullable ClickType click, boolean inputSlot, boolean cursorEmpty, boolean clickedEmpty)`.
- `static Decision drag(PlayerInventoryPolicy policy, int topSlots, boolean inputSlot, boolean cursorEmpty)`.
...
| TOP cell, anything else (plain cell, empty cursor, shift/middle/number-key/drop/offhand) | `CANCEL_AND_CLICK` |
| OUTSIDE the window, either policy | `CANCEL_ONLY` |
| BOTTOM, policy `LOCKED` | `CANCEL_ONLY` |
| BOTTOM, policy `OPEN`, SHIFT_LEFT/SHIFT_RIGHT over a non-empty stack | `OFFER_SHIFT` |
...
| DRAG covering exactly ONE menu cell that is an input cell, dragged stack non-empty | `OFFER_DRAG` |
| DRAG, anything else (2+ cells even if all are input cells; one non-input cell; empty cursor) | `CANCEL_ONLY` |

Two invariants live here and everything else follows from them: `COLLECT_TO_CURSOR` is cancelled unconditionally BEFORE anything else (it is the one action that pulls stacks out of the TOP inventory while the click that fires it lands on the bottom one), and a click on a cell of the MENU is always cancelled - as a plain click or as an offer, it makes no difference. The second is what makes leaving the player's own inventory alone safe: no rendered stack can reach the cursor, so nothing the player then does with their own inventory can be moving a GUI item. Deliberate details: a shift-click over an EMPTY bottom slot passes through rather than being cancelled (vanilla does nothing with it, and cancelling a no-op only costs a client desync); a drag spread over several input cells is cancelled rather than split, because inventing a split would be SnLib deciding how much of the stack each cell gets; and offers never pass through the menu's `strict-clicks` gate, which filters ACTIONS and an offer is not one.
...
- Limitations documented in the code:
  ...
  - A menu file named exactly like a merge backup (`old-<name>-<8 digits>-<6 digits>.yml`, any case) is never loaded (1.39.0): that name is reserved for the backups `YamlUpdater` writes.
  ...
  - A plugin-supplied stack (1.21.0) takes only `display-name` and `lore` from its template. That is deliberate - the stack is the authority on its own appearance - but it does mean the server owner cannot restyle such a cell's material, glow or model data from the yml; only its name, its extra lore lines and its behaviour. A cell whose appearance the owner should own fully is a plain template bind, not a supplied stack.
  - There is no supplied-stack variant of the no-slot `bind(String templateId, Ph... phs)`: that overload renders the SAME bind into every cell of a template's key, which is the one placement where a per-entry appearance has no meaning. A group of cells that needs one distinct stack each is a region (`bindEach` + `GuiEntry.stack`).
  - A supplied stack is bound to a TEMPLATE, always: there is no `bindStack(slot, stack)` without one, because the template is what carries the click behaviour and a slot with no definition behind it would render but never respond.
  - An input cell (1.28.0) needs `player-inventory: open` to be reachable: with the player inventory locked the viewer can never pick a stack up, so their cursor stays empty and no offer of any kind can be produced. The parse WARNs about the combination rather than opening the inventory on the owner's behalf.
  - SnLib never ACCEPTS an offer, only reports it. There is no "put the stack in the cell" helper, and there will not be one: the amount taken, the remainder, where it is stored and what the cell then shows are all consumer decisions, and a library that guessed them would have to own the money-shaped half of every deposit flow. The consumer writes the remainder back itself and follows it with `viewer.updateInventory()` (see `ItemOffer`).
  - A drag spread over several input cells is cancelled, not split. One drag, one target cell, or nothing.
  - The supplied-stack path is sized for a declarative per-viewer session (opens, refreshes, page changes, `update-interval:` ticks), NOT for a per-frame full-strip repaint at animation rates. It costs one `ItemStack` clone per rendered cell plus one record per bind - cheaper than the template render it replaces, but not free. A 10 Hz animation belongs on a raw Bukkit inventory; the exact per-bind and per-render cost is itemized under `StackOverlay` so the next consumer can make that call from the numbers.
---

## 13. Commands

Command module of the consumer context, reached via `sn.commands()`. It provides a fluent root/subcommand tree builder (`SnCommands` -> `RootBuilder` -> `SubCommandBuilder`) that materializes into a Bukkit `RootCommand` with permission check first, argument count validation against the generated usage, typed parsing per `Arg` and generated paginated help with `Page`. Subcommands NEST (1.6): a subcommand that owns children (declared through `SubCommandBuilder.sub(name, spec)`) is a GROUP that dispatches on the next token (`/clan admin disband <clan>`); a childless subcommand is a LEAF that parses its positional args. Every root injects the `reload` and `help` subcommands by default (and `debug` if the spec declared it), replaceable or omittable with `withoutDefaults()`; a bare-root invocation runs the optional `onEmpty` hook or, without one, the generated help. A root may name one root-level leaf as its FALLBACK (`RootBuilder.fallbackSub(name)`, 1.37.0): a first token that matches no declared subcommand becomes that leaf's first argument, so `/trade Steve` runs `/trade request Steve`. A root registered at runtime on purpose, never declared in the plugin.yml, is marked with `RootBuilder.dynamic()` (1.38.0): no "not declared" WARN for it or its aliases, and a name another plugin holds is noted once at INFO naming the `/<plugin>:<name>` form it still answers on. Registration against Bukkit is done by `internal/BukkitCommandRegistry` with `Plugin` ownership (reload-safe, tenant sweep when the consumer disables) and client tree refresh via `updateCommands()`. The library itself registers its diagnostic `/snlib` command through this same path (`internal/SnLibCommand`). All execution and tab-complete run on the server's main thread (standard Bukkit dispatch).
...
- `public SnCommands(Sn ctx, @Nullable SnLang lang, boolean debugCommand)` - constructor; instantiated by the context. `lang` may be null (the shared default `snlib.*` templates render); `debugCommand` says whether the spec declared the debug command. Since 1.39.0 it delegates with `commandPriority = false`.
- `public SnCommands(Sn ctx, @Nullable SnLang lang, boolean debugCommand, boolean commandPriority)` (1.39.0, API level 26) - the constructor `Sn` calls, with `spec.commandPriority()`; every root this module builds passes the flag to `BukkitCommandRegistry.bindAliasSupplier(command, supplier, dynamic, commandPriority)`.
...
- `public RootBuilder fallbackSub(String name)` (1.37.0) - names the root-level LEAF that takes over a first token matching no declared subcommand or alias, with EVERY token as its arguments: `fallbackSub("request")` makes `/trade Steve` run `/trade request Steve`. Declared subcommands always win (a player literally named `accept` is reached through `/trade request accept`); the bare root keeps its `onEmpty` hook or help; a sender without the fallback's own permission gets the plain `snlib.unknown-subcommand`. Tab offers the fallback's first-argument suggestions next to the subcommand names, and usage errors reached through the shortcut and the fallback's help entry render the short form (`/trade <player>`). Name trimmed and lowercased; null throws `NullPointerException`, blank throws `IllegalArgumentException` ("Empty subcommand name"). The name is resolved by `register()`. See "Root fallback" under `RootCommand`.
- `public RootBuilder dynamic()` (1.38.0, API level 25) - marks the root as registered at runtime ON PURPOSE and never declared in the plugin.yml (a command whose name comes from the owner's config or content files, like SnDungeons' currency and module roots). Neither the root nor its aliases log the "not declared in the plugin.yml" WARN, at enable or on any reload. When another command already holds the name or an alias, the existing one is kept (as always), the root keeps answering on its `/<plugin>:<key>` form, and the collision is logged ONCE per owner and key per enable at INFO instead of a WARN on every pass: `Command '/money' of SnDungeons is taken by Essentials; kept it. This command answers as /sndungeons:money` (alias form: `Alias '/bal' of '/money' in SnDungeons is taken by Essentials; kept it. This alias answers as /sndungeons:bal`). A key whose namespaced form is ALSO held by someone else leaves the command unreachable under it and keeps the pre-1.38.0 WARN. No effect when the plugin.yml declares the name (Bukkit registers that root, as without the call). Stored in the registry state, not in `RootCommand`, see `BukkitCommandRegistry`. Under the spec's `commandPriority()` (1.39.0) a key held by another plugin is taken instead of kept.
- `public RootCommand register()` - builds the tree, resolves the `fallbackSub` name (1.37.0; `IllegalStateException` when it matches no declared root-level subcommand by name or alias, or names a group, checked before anything is registered), injects the applicable defaults, binds the alias supplier and the `dynamic()` flag to the built root (`BukkitCommandRegistry.bindAliasSupplier`, 1.5.0; the three-argument overload with the flag since 1.38.0) and registers it against Bukkit; returns the `RootCommand`.
...
- `fallbackSub` only accepts a subcommand the consumer DECLARED on this root: the injected `reload`/`help`/`debug` are added later by the `RootCommand` constructor and never qualify.
...
- `DEFAULT_MESSAGES` - static map of default templates mirroring `snlib-messages.yml` (server-wide static justified by being constant). Keys: `snlib.no-permission`, `snlib.usage`, `snlib.invalid-number`, `snlib.invalid-value`, `snlib.out-of-range`, `snlib.player-not-found`, `snlib.unknown-subcommand`, `snlib.reload-done`, `snlib.help.header`, `snlib.help.entry` (default `&e{usage} &7{description}`, no `&8:` separator since 1.6), `snlib.help.footer`, `snlib.help.group-header` and `snlib.help.group-footer` (1.36.0).
- `HELP_PAGE_SIZE = 10` - entries per page of the generated help (the group help included).
- `GROUP_HELP_TOKEN = "help"` (1.36.0) - child token that opens the generated help of a `groupHelp()` group.
- Constructor takes an extra `@Nullable Consumer<RootContext> onEmpty` (1.6) after the defaults/debug flags, and a trailing `@Nullable Sub fallback` (1.37.0), already resolved and validated by `RootBuilder.register()` through the static `fallbackOf(rootName, declared, fallbackName)`.
...
3. `resolve` finds the first-token node by name or alias (lowercase); unknown -> `Message("snlib.unknown-subcommand", {value})`, unless the root has a fallback the sender may use (1.37.0), in which case the fallback leaf is dispatched with `matchedAt = -1` (every token is its argument) under the bare root path (see "Root fallback" below). Then it recurses through `dispatch(sender, sub, args, matchedAt, path)`:
   - Node permission gate (its own; the chain is enforced node by node as the tree descends) -> `snlib.no-permission` on failure.
   - GROUP (has children): with no next token -> `snlib.usage` with the group usage `/path <childA|childB>` (only the children the sender may use); an unknown next token -> `snlib.unknown-subcommand` with the full path; otherwise recurse into the matched child. A group declared with `groupHelp()` (1.36.0) resolves to `GroupHelp(group, path, page)` instead of the usage when it gets no next token (page 1), and on a `help [page]` token (case-insensitive) that no declared child claims; the page parses like the root help's (unparseable -> 1). `execute` renders it through `sendGroupHelp`.
...
- `tabAt` recurses: at group depth it suggests the visible child names whose permission the sender holds, filtered by prefix and sorted (`suggestNames`); descending into a group re-checks that group's permission. A `groupHelp()` group also suggests `help` while its child token is typed, unless a declared child owns that name (1.36.0). At leaf depth (`tabLeaf`) it completes the positional arg: if the index exceeds the declared args only a final greedy arg keeps suggesting; otherwise it delegates to the positional arg's `Arg.suggest(sender, partial, argName)` (1.5.0), passing the declared name so a free-form arg suggests its `<argName>` hint. A `suggest` returning null normalizes to an empty list.
- With a root fallback the sender may use (1.37.0), the static overload `tab(sender, rootPermission, subs, fallback, args)` adds the fallback's first-argument suggestions after the node names at the first token, and completes the fallback's arguments at later tokens whenever the first token matches no declared node (see "Root fallback" below).
...
**Group help (1.36.0)**
A group declared with `SubCommandBuilder.groupHelp()` gets its own generated help and stops flooding its parent's. In the parent's help `collectHelp` yields ONE line for the whole group: usage `<group path> help`, the group's (lang-resolved) description, and the group's effective permission; the line is emitted only when the group's own help would list at least one leaf for that sender. The group's own help (`sendGroupHelp`) renders `snlib.help.group-header` (`{group}`, `{path}`, `{description}`, `{plugin}`), then `collectHelp` over the group's children only (full paths under the typed label; a nested `groupHelp()` group collapses the same way), each through `snlib.help.entry` with the effective permission carried from dispatch (the group's own, or the nearest ancestor's), paginated by 10, and `snlib.help.group-footer` (`{page}`, `{total}`, `{path}`) only with more than one page. The group's permission is enforced before any of it (`dispatch` checks it first). Intended use: a modular plugin with one admin group per module (`/dg admin <module> help`), which before 1.36.0 had to hand-write a scoped help leaf and hide the module leaves with `helpVisible(false)`. A child declared with the name `help` wins over the generated help (dispatch, tab and help alike), and `groupHelp()` on a node without children is ignored (`Sub.groupHelp` is stored as `groupHelp && !children.isEmpty()`).

**Root fallback (1.37.0)**
`RootBuilder.fallbackSub(name)` names one declared root-level LEAF (by name or alias) that takes over a first token no declared node claims, so `/trade Steve` behaves like `/trade request Steve` (SnTrade). The static `fallbackOf(rootName, declared, fallbackName)` resolves it at `register()` and throws `IllegalStateException` for a name that matches no declared root-level subcommand or that names a group; the resolved `Sub` is held by identity.

```java
sn.commands().root("trade")
        .sub("request").arg("player", Args.onlinePlayer()).executes(this::request).and()
        .sub("accept").arg("player", Args.onlinePlayer()).executes(this::accept).and()
        .fallbackSub("request")          // /trade Steve == /trade request Steve
        .register();
```

Contract:
- Resolution: only when `args.length >= 1` and `find(subs, args[0])` is null. The root permission is checked first as always; then, if the sender holds the fallback's OWN permission (`mayUse`), `dispatch(sender, fallback, args, -1, rootPath, rootPermission)` parses EVERY token as the fallback's positional arguments; otherwise the result stays `snlib.unknown-subcommand` with the bare token, byte-identical to a root without a fallback, so the fallback is never revealed to a sender who may not use it. Declared nodes always win, including an alias, a hidden (`visible(false)`) node and a node whose permission the sender lacks (that one still answers `snlib.no-permission`). Zero arguments still resolve to `Empty`.
- Rendering: the arity, `when` and missing-executor usage lines of that dispatch render under the bare root path (`/trade <player>`), because that is what the sender typed; `/trade request ...` keeps the full path. Parse errors keep their own key (`snlib.player-not-found` and so on). An explicit `usage(...)` is a literal and renders as written in both forms. The resolved `Run` still carries the full path (`/trade request`) for the "Subcommand ... failed" log, and `CommandContext.label()` is the typed label as usual.
- Tab: at the first token, the sorted node names (`suggestNames`, unchanged) followed by the fallback's first-argument suggestions (`tabLeaf` with `argsStart = 0`, so the arg's own filter-then-cap order applies), each kept only when it starts with the typed token (case-insensitive) and is not already offered (case-insensitive, so a player named `Accept` does not repeat the `accept` subcommand). At a later token with an unmatched first token, `tabLeaf(sender, fallback, args, 0)` completes the fallback's arguments. A matched first token completes exactly as without a fallback. All of it only when the sender holds the fallback's permission; its `visible` flag does not gate the suggestions.
- Help: `collectHelp(sender, nodes, fallback, path, inheritedPermission)` renders the fallback's single line as `usageOf(fallback, path)` (`/trade <player> [note...]`) instead of `/trade request <player> [note...]`, still filtered by `visible`, `helpVisible` and permission like every other leaf.
- A root without `fallbackSub` passes a null fallback everywhere, and the pre-1.37.0 static overloads (`resolve`, `tab`, `collectHelp` without the fallback parameter) delegate with null, so its behavior is unchanged. Covered by `FallbackSubCommandTest`.
...
- `resolve` returns a sealed `Resolution` (`Empty`, `Message`, `GroupHelp` since 1.36.0, `Run`), so the outcome is decided independently of the Bukkit context and unit-tested without a server (`NestedCommandTest`, and `FallbackSubCommandTest` for the root fallback since 1.37.0).
...
Immutable subcommand node built by `SubCommandBuilder`: `name` (trim + lowercase), `aliases` (lowercased, immutable copy), nullable `permission`, nullable `usage`, `description`, `visible`, `helpVisible` (1.10.0), `args` (immutable `LinkedHashMap`, declaration order is parse order), `requiredArgs`, `conditions`, nullable `executor`, `children` (1.6, immutable copy; a non-empty list makes the node a GROUP that dispatches instead of running its args/executor) and `groupHelp` (1.36.0, true only for a group). `static Sub of(String name, @Nullable String permission, String description, Consumer<CommandContext> executor)` fabricates the default subs (argless, visible, help-visible, no children, no group help).

The `execute` outcome is a sealed `Resolution permits Empty, Message, GroupHelp, Run` (1.6; `GroupHelp` 1.36.0): `Empty` (zero args, the `onEmpty` hook or help applies), `Message(key, phs)` (a lang key to send), `GroupHelp(group, path, permission, page)` (the own help of a `groupHelp()` group to render, with its effective permission), `Run(sub, context, path)` (the resolved leaf ready to run). `HelpLine(usage, description, permission)` is the flattened help record.
...
- `public SubCommandBuilder groupHelp()` (1.36.0) - gives this GROUP its own generated help: the bare group (`/dg admin party`) and `/dg admin party help [page]` list only this group's reachable leaves, paginated, under the typed label, through `snlib.help.group-header` / `snlib.help.entry` / `snlib.help.group-footer`; the parent's help shows ONE entry for the whole group (`/dg admin party help` with the group's description); tab completion offers `help` among the children. A declared child named `help` wins; no effect on a node without children. Replaces the hand-written scoped help leaf (the SnMiniGames `GameAdminSection` pattern) for new plugins; see "Group help" under `RootCommand`.
...
Factory of typed `Arg` implementations for `SubCommandBuilder.arg`. Every arg built here carries default example suggestions and accepts the `suggestCurrent(Supplier)` decorator, which prepends the real current value: with an empty partial (or a matching prefix) the current value and the examples go first; a non-empty partial filters the FULL base option set via `StringUtil.copyPartialMatches`, sorts the matches, and only then caps them (1.34.1 - until then the cap was applied to the option set BEFORE the filter, so an option past the cap could never be reached by typing its own prefix). The vanilla client filters suggestions by the typed prefix, so an example that does not match never reaches the screen.

Constant:
- `SUGGESTION_CAP = 500` (private) - cap on list-backed suggestions (online players, `oneOf` / `suggesting` options), applied to the prefix MATCHES, never to the option set. Was 100 and pre-filter until 1.34.1.

Factory methods (all `static`):
- `public static SnArg<Player> onlinePlayer()` - online player by exact name (`Bukkit.getPlayerExact`); rejects with `snlib.player-not-found` (`{value}`) and suggests the online names matching the typed prefix (cap 500).
...
- `public static SnArg<Integer> intRange(int min, int max)` - integer in `[min, max]`; a non-number or non-integral value rejects with `snlib.invalid-number` and out-of-range with `snlib.out-of-range` (`{value}`, `{min}`, `{max}`). Suggests both bounds as examples, so the bounds must be human-meaningful; an open-ended amount belongs to `intMin`, never to a sentinel like `Integer.MAX_VALUE`.
- `public static SnArg<Double> doubleRange(double min, double max)` - double in `[min, max]`; non-number -> `snlib.invalid-number`; out of range -> `snlib.out-of-range`. Suggests both bounds; same human-meaningful-bounds rule (an ugly domain cap belongs to `doubleMin`).
- `public static SnArg<Integer> intMin(int min)` (1.12.0) - integer of at least `min` with NO upper bound (open-ended amounts: withdraw, give, pay). Its lone suggestion is the angle-bracket `<argName>` hint (hint mode, like the free-form factories); a non-number or non-integral value rejects with `snlib.invalid-number` and a value under `min` with `snlib.number-too-small` (`{value}`, `{min}`).
- `public static SnArg<Double> doubleMin(double min)` (1.12.0) - double of at least `min` with NO upper bound; same suggestion and rejection contract as `intMin`.
- All four numeric factories (1.12.0) parse through `NumberFormatter.parseFormatted`, so abbreviated input with the case-insensitive suffixes `k/m/b/t/qa/qi` is accepted everywhere (`2k` = 2000, `1.5b` = 1.5e9) and comma/dot separators normalize per that contract (`1,500` groups to 1500; `1,5` reads as 1.5). NaN/infinite reject with `snlib.invalid-number` (private helpers `parseNumericToken`/`parseIntToken`).
...
Bridge between the `RootCommand` trees and Bukkit's command system, preferring the public API through two paths: (a) a command declared in the owner's plugin.yml receives its executor and tab completer via `plugin.getCommand(name)` (the `PluginCommandAdapter` adapter); (b) undeclared roots go through Paper's public `Bukkit.getCommandMap()`, with a WARN unless the root was built with `dynamic()` (1.38.0). In BOTH paths the dynamic aliases (builder varargs, an alias supplier, or the config-driven binding) are RECONCILED against the CommandMap's known commands (1.5.0). After EVERY register and unregister, online players receive `updateCommands()` so their client trees never show ghosts; a pass that actually MUTATED the map goes through `CommandSync` instead (1.33.1), which re-syncs the server's own dispatcher before the trees are re-sent. Registered roots track in a `TenantRegistry<RootCommand>` (justified server-wide static) keyed by owning plugin: the tenant sweep detaches each command and removes the owner's whole key when the consumer disables. Dynamic aliases are re-sourced on every register pass (the reload flow re-registers the same root instance): the alias supplier is re-evaluated, aliases that appeared are CLAIMED when their key is free (with the plugin.yml WARN on the fallback path ONLY, 1.12.1), and aliases that disappeared are removed. The supplier is stored in a `Map<RootCommand, RootState>` (`STATES`; the state class was `AliasState` until 1.38.0, which added the `dynamic` flag and the held namespaced alias keys) keyed by root identity, so the `RootCommand` core stays immutable; since 1.33.1 that map is weakly keyed (`Collections.synchronizedMap(new WeakHashMap<>())`) because a detached root KEEPS its binding. The INFO collision notes of `dynamic()` roots are deduplicated in a second justified server-wide static, `TenantRegistry<String> COLLISIONS_NOTED` (1.38.0), keyed by owner and lowercased key, NEVER by root instance (a consumer such as SnDungeons rebuilds its dynamic roots on every reload); `Sn.shutdown` sweeps it with every other tenant registry, so each enable notes each collision once.

**Writing into `getKnownCommands()` (1.33.1)** - every read and write uses the map's own `get`, `put` and two-arg `remove`. On Paper 1.20.6+ `CraftCommandMap` is constructed with `BukkitBrigForwardingMap.INSTANCE`, a `HashMap` subclass that forwards only its OVERRIDDEN methods to the Brigadier dispatcher; `putIfAbsent` is not among them, so it writes to the dead inherited table and returns null as if the insert had succeeded, and the entry-set iterator streams the dispatcher and rejects `remove`, so `entrySet().removeIf` throws once a predicate matches. Both were in use before 1.33.1, which is why no config-driven alias was ever dispatchable on those versions - not even its `plugin:alias` form, and not at cold boot either.

- `public static void bindAliasSupplier(RootCommand command, @Nullable Supplier<Collection<String>> supplier)` (1.5.0) - binds the dynamic-alias supplier of a root before it is registered; a null supplier means the builder / plugin.yml aliases are the sole source. Called by the command builder at build time so the supplier travels with the root into every register pass. Since 1.38.0 it delegates to the overload below with `dynamic = false`.
- `public static void bindAliasSupplier(RootCommand command, @Nullable Supplier<Collection<String>> supplier, boolean dynamic)` (1.38.0) - the same binding plus the `dynamic()` flag; since 1.39.0 it delegates with `priority = false`.
- `public static void bindAliasSupplier(RootCommand command, @Nullable Supplier<Collection<String>> supplier, boolean dynamic, boolean priority)` (1.39.0) - the same binding plus the owner's command priority, stored as `new RootState(supplier, dynamic, priority)`; what `RootBuilder.register()` calls.
- `public static void reclaimPriority()` (1.39.0) - `reclaimPriority(null)`, what `ServerLoadEvent` calls.
- `public static void reclaimPriority(@Nullable Plugin enabled)` (1.39.0) - for every registered root whose state has `priority`, takes back each bare key another plugin overwrote since (a later plugin whose LABEL equals one of its alias keys: `SimpleCommandMap.register` only refuses a key whose current command has that same label, and a root's label is its name, so its name key is safe and its alias keys are not; or a direct write into the known commands). Checks the name (declared or fallback path) and every key in `state.active` one by one against the map with `get`, because `reconcileAliases` returns early when the alias diff is empty. A key found empty is left alone (the owner may have cleared it on purpose, e.g. a `commands.yml` alias with no target). Main thread; publishes once through `refresh(true)`, and only when a key moved. With a plugin that just enabled, `noteLatecomer` then runs. Called by `CommandPriorityListener`.
- `noteLatecomer(roots, enabled)` (1.39.0) - for each bare key a priority root holds (its name and its `active` aliases, when the map still points at the root) with NO entry in `state.displaced`, when `known.get(<enabled, lowercased>:<key>)` is a command `enabled` provides, records it as displaced, silently and without writing the map: that plugin enabled after the root and Bukkit refused it the bare key, so it gets the key when the root releases it.
- `public static void register(JavaPlugin owner, RootCommand command)` - registers the root for its owner. Reload-safe: a root already registered by the same owner under the same name detaches and replaces first; re-registering the SAME instance keeps it in place and only reconciles its dynamic aliases. Reads the root's `RootState` (`computeIfAbsent`, default `(null, false)`); the effective flag is `state.dynamic && declared == null`. The plugin.yml path if the command is declared; otherwise a WARN ("Command '/x' not declared in the plugin.yml of Y; dynamic registration via CommandMap"), skipped for a `dynamic()` root (1.38.0), and CommandMap registration with prefix = the owner's lowercased name, claiming `name` and `prefix:name` - since 1.33.1 that path also reports when the root NAME itself is already taken (previously a silent no-op that left the command dead): with `ownsNamespace = takenNamespaced == null || takenNamespaced == command`, `CollisionNotice.of(dynamic, ownsNamespace, alreadyNoted)` picks the pre-1.38.0 WARN ("Command '/x' of Y collides with an existing command; kept the existing one") or, for a reachable `dynamic()` root, one INFO per owner and key (see `CollisionNotice` below). Then `reconcileAliases`, then `refresh(mutated)`.
- `public static void unregister(JavaPlugin owner, RootCommand command)` - unregisters an owner's root and refreshes the client trees. Since 1.39.0, when the root had command priority and the detach mutated the map, the owner's remaining roots run `reclaimRoots` first: a key the root gave back may be the name or an active alias of a sibling root.
...
**Internal logic**
- `sweep(RootCommand)` - the `TenantRegistry` callback: `refresh(detach(command))`; it also runs when the tenant sweeper removes a disabled owner's key.
- `reconcileAliases(owner, command, declared, state, dynamic)` (1.5.0, replaces `registerDynamicAliases`; the state and the effective `dynamic` flag are passed in since 1.38.0) - the desired set is the alias supplier's value when it has one (authoritative, config-driven), otherwise the builder / plugin.yml aliases; the root name and the plugin.yml declared aliases are always excluded (via `AliasReconciler.resolve`). It diffs the previous pass's `active` set against the desired one (`AliasReconciler.diff`): removed aliases are dropped from `getKnownCommands()` (both `alias` and `owner:alias`), added aliases are claimed with `claim`. An alias whose key is already held by another command is a COLLISION, kept out and reported through `CollisionNotice` (1.38.0): the aliases whose notice is WARN share one line ("Aliases [...] of '/x' collide with existing commands; kept the existing ones", the pre-1.38.0 text), while each alias of a `dynamic()` root that still owns its `prefix:alias` form gets one INFO per owner and key. The rest WARN ("Aliases [...] of '/x' not declared in the plugin.yml of Y; dynamic registration via CommandMap") **only on the fallback path of a root that is not `dynamic()`** (1.12.1 / 1.38.0, via `AliasReconciler.warnsUndeclared(dynamic, supplied, added)`): when the supplier answered (config binding / `aliases(Supplier)`) the aliases are runtime-owned and CANNOT be declared in the plugin.yml, so they register silently instead of warning on every boot. Returns whether the map was mutated (1.33.1); `active` records the desired set MINUS the collided aliases, so a collision is retried on the next pass instead of being remembered as registered. Namespaced leak fix (1.38.0): a collided alias still claims `prefix:alias` when that key is free, but was never in `active`, so neither `detach` nor the removed diff ever released it and the `/plugin:alias` key outlived the root as a ghost; each pass now records those aliases in `RootState.namespaced`, releases (two-arg remove) the held keys whose alias left the desired set (`AliasReconciler.diff(state.namespaced, desired).removed()`, checked before the early "nothing changed" return), and `detach` releases them all.
- `claim(known, key, command)` (1.33.1) - claims the key for the root when `known.get(key)` is null, returning the occupant that kept it or null when the claim landed. Replaces `putIfAbsent`, which does not reach the dispatcher on Paper 1.20.6+. Since 1.39.0 it claims only the namespaced `prefix:key` forms, which are never taken.
- `claimOrTake(owner, known, key, command, state)` (1.39.0) - `claim` of a BARE key (the root name on the fallback path, each alias in `reconcileAliases`): under `state.priority`, an occupant `CollisionNotice.takesOver` accepts is replaced (`take`) and the key counts as claimed (the alias enters `active`, the fallback root logs no collision). Without priority, or against any other occupant, it is exactly `claim` (the occupant is not even classified), so the 1.38.0 INFO/WARN follows.
- `takeDeclared(owner, known, command, declared, state)` (1.39.0) - the declared path under priority: when Bukkit left the bare name of the plugin.yml command with another plugin (the declared `PluginCommand` then answers as `plugin:name`), the `RootCommand` itself goes into the bare key (its label IS the name, so a later plugin registering that label cannot replace it) and the occupant is displaced; the declared command keeps its namespaced key and its executor adapter. `JavaPlugin.getCommand(name)` still finds the declared command through its namespaced key.
- `reclaimRoot(owner, command, state)` / `retake(...)` (1.39.0) - the per-root half of `reclaimPriority`: the name (declared path through `takeDeclared`, so only when the owner's plugin.yml command does not hold it; fallback path through `retake`) and each alias in `state.active`, taken from an occupant `takes` accepts; nothing is logged for a key it does not take. `register` runs it at the END of every pass of a priority root (after `reconcileAliases`, which returns early on an empty alias diff), so a reload of the owner takes its keys back.
- `takes(owner, command, key, holder)` (1.39.0) - the takeover test of the three paths: never a key holding `:` (a namespaced form stays its plugin's even when a config lists it as an alias), otherwise `CollisionNotice.takesOver(true, occupantKind(...))`.
- `reclaimRoots(roots)` (1.39.0) - `reclaim` of every priority root among `roots`, each isolated in a `Throwable` catch (the owner logs `Could not take back the keys of '/<name>' (command priority): <error>` and the pass continues, counted as mutated); shared by `reclaimPriority` and `unregister`.
- `take(owner, known, key, command, state, occupant)` (1.39.0) - `known.put(key, command)`, `state.displaced.put(key, new Displaced(<occupant's plugin name>, new WeakReference<>(occupant)))` (the LAST command displaced from that key) and `noteTakeover`.
- `noteTakeover(owner, known, command, key, occupant)` (1.39.0) - ONE INFO per owner and key per enable, recorded in `COLLISIONS_NOTED` as `"priority:" + key`, apart from the 1.38.0 collision note of the same key (so a later loss of that key to a server command is still reported): `CollisionNotice.rootTakeover` for the root name, `aliasTakeover` for an alias. The clause naming the displaced command's namespaced form is added only when `known.get(<its plugin, lowercased>:<key>)` is that same command.
- `occupantKind(owner, command, occupant)` (package-private, 1.39.0) - classifies the holder of a key with `org.bukkit` types only (no reflection, no craftbukkit), in this order: the root itself, a root of the same owner, the `PluginCommandAdapter` of one, or a command the owner's plugin provides -> `OWN`; a `RootCommand` of another owner, or a `PluginCommand` whose executor is the `PluginCommandAdapter` of such a root (a DECLARED root of another consumer) -> `SNLIB_ROOT` or `SNLIB_PRIORITY_ROOT` by that root's own `RootState.priority` (no state -> `SNLIB_ROOT`); otherwise by `pluginOf`: another plugin -> `PLUGIN`, none -> `SERVER`. Tested by `OccupantKindTest`.
- `pluginOf(command)` (1.39.0) - the plugin that provides a command: the owner of a root (or of the root an adapter runs); `getPlugin()` of a `PluginIdentifiableCommand` that is NOT a `BukkitCommand` (null -> none); otherwise `JavaPlugin.getProvidingPlugin(command.getClass())`, so a plugin registering its own `Command` or `BukkitCommand` class counts as that plugin, "absolute" priority over every plugin command. A class the server loaded throws (`IllegalArgumentException`, or `IllegalStateException` from a static initializer) -> none: `VanillaCommandWrapper`, `PluginVanillaCommandWrapper` (a Brigadier command, which names a plugin but is a class of the server), Bukkit's defaults (`version`, `plugins`, `reload`, `help`, `timings`), `FormattedCommandAlias` (`commands.yml`), `PaperCommand`, `MSPTCommand`, `SpigotCommand`, `TicksPerSecondCommand`, `RestartCommand` all stay `SERVER`.
- `release(known, key, command, state)` (1.39.0) - replaces the bare-key `known.remove(key, command)` of `detach` and of the removed aliases in `reconcileAliases`: removes the root from the key and, when it held it, hands the key back through `restore`; the displaced entry is forgotten either way. Without priority nothing was ever displaced, so it is the old two-arg remove.
- `restore(known, key, displaced)` (1.39.0) - silent: nothing while `Bukkit.isStopping()` or when the key is no longer empty. The displaced command gets the key when it was not collected and is `returnable`; otherwise the live command at `<displaced plugin, lowercased>:<key>` does when it is `returnable` (a plugin that re-registered its commands, or that a plugin manager reloaded, holds a new command object); otherwise nobody does.
- `returnable(known, command, plugin, key)` (1.39.0) - the command must be provided (`pluginOf`) by an ENABLED plugin named `plugin`, so never a class of the server; a `RootCommand` must still be registered (`COMMANDS.forOwner(owner).contains(root)`) and still want the key (its name, an alias in its `active`, or an alias in its `namespaced`), so a key never points at a root that unregistered or dropped it; any other command must still answer at `<plugin>:<key>` (so a command its plugin dropped is never put back).
- `adopt(root, key)` (1.39.0) - after a key that is not its name went back to a root, adds it to the root's `active` (a new immutable list, and drops it from `namespaced`) so the root's own detach or alias reconcile releases it.
- `Displaced` (private record, 1.39.0) - `(String plugin, WeakReference<Command> command)`: the displaced command is held weakly so a plugin a manager unloads is never kept alive through the record (a live command stays strongly reachable through the known commands); once collected, `restore` uses the live namespaced form.
...
- `detach(RootCommand)` - unhooks the command from whichever path registered it: removes BY KEY (1.33.1) each active alias and its `owner:alias` form, the `owner:alias` form held by each collided alias (`RootState.namespaced`, 1.38.0), plus the root name and its `owner:name` form, each via the two-arg `known.remove(key, command)` so an entry owned by something else is never touched; calls `command.unregister(map)`; and if the declared `PluginCommand`'s executor is a `PluginCommandAdapter` of THIS root, clears the executor and tab completer. Resets `RootState.active` and `RootState.namespaced` but KEEPS the supplier binding and the `dynamic` flag, so the same instance can be detached and registered again without silently losing its config-driven aliases. Returns whether the map was mutated.
- `refresh(boolean mutated)` (1.33.1) - a mutated pass goes to `CommandSync.sync()`, and only falls through to `updateCommands()` when that reports it did not run; an unmutated pass refreshes the client trees directly, so boot never pays for a dispatcher rebuild it does not need.
- `updateCommands()` - `player.updateCommands()` for every online player (main thread).
- `notice(owner, key, dynamic, ownsNamespace)` (1.38.0) - `CollisionNotice.of(dynamic, ownsNamespace, COLLISIONS_NOTED.forOwner(owner).contains(lowercased key))`; an INFO outcome records the key in `COLLISIONS_NOTED` so the later passes of the enable resolve to NONE. Main thread only, like every register pass, so the check-then-add needs no further coordination.
- `occupantOf(Command)` (1.38.0) - the plugin named by the INFO: `PluginIdentifiableCommand.getPlugin().getName()` (a plugin.yml command of Essentials and the like), else `RootCommand.owner().getName()` (a root of another SnLib consumer, which is not a `PluginIdentifiableCommand`), else `another command`.
- `RootState` (private static final class; `AliasState` until 1.38.0) - per-root registration state: the `supplier`, the `dynamic` flag, the `priority` flag (1.39.0), the `volatile List<String> active` currently-registered alias keys, the `volatile List<String> namespaced` aliases that lost their bare key but hold their `prefix:alias` form, and the `Map<String, Displaced> displaced` (1.39.0, a `ConcurrentHashMap`: lowercased bare key -> the last command displaced from it), cleared by `detach`.
...
**Notes and gotchas**
- Claiming a dynamic alias never overwrites: an alias already taken by another command is left with the existing command and reported as a collision, not operative for this root under its bare key (its `prefix:alias` form still works when free). The one exception is a plugin with command priority (1.39.0), whose bare keys are taken from other plugins; namespaced keys are never taken.
...
- Never reach for a `Map` default method on `getKnownCommands()`. Only what `BukkitBrigForwardingMap` overrides reaches Paper 1.20.6+'s dispatcher; anything else (`putIfAbsent`, `compute*`, `merge`, `replace`) silently hits the dead inherited `HashMap` table, and the collection views reject removal.

#### CommandSync (internal, 1.33.1)
`src/main/java/com/sn/lib/command/internal/CommandSync.java`

Reflective bridge to the server's `CraftServer#syncCommands()`, used by `BukkitCommandRegistry.refresh` after a pass that actually mutated the CommandMap. Package-private final class with a private constructor (statics only), so it adds nothing to the public surface.

- `static boolean sync()` - invokes the method on `Bukkit.getServer()` and reports whether it ran; a false return leaves the caller owing the online players a `updateCommands()` refresh. Returns false immediately off the main thread or while `Bukkit.isStopping()` - the map write itself has already landed, and a shutdown teardown has no tree left to refresh. Never throws: an invocation failure is logged at FINE and degrades to the client-tree refresh.
- `resolve()` - walks up from `Bukkit.getServer().getClass()` calling `getDeclaredMethod("syncCommands")` until it finds the method, caching it (or the absence) in a `volatile` static so the walk runs at most once per server session. Walking beats naming a package: it works on Paper's unversioned `org.bukkit.craftbukkit.CraftServer`, on a relocated `org.bukkit.craftbukkit.vX_Y_RZ.CraftServer`, and on any fork that subclasses either, with no NMS version string in the code. A server exposing no such method is reported once at FINE, not WARN: that is a deliberate environment, not a misconfiguration.

What the call does is version-dependent, and both ends are the point: on Paper 1.20.4 / 1.20.5 it REBUILDS the Brigadier dispatcher from the known commands (the server itself only calls it once, at the end of `enablePlugins(POSTWORLD)`, which is why cold boot worked there and the reload path did not); on Paper 1.20.6+ the forwarding map has already published the write and the call only re-sends the client trees.

#### CollisionNotice (internal, 1.38.0)
`src/main/java/com/sn/lib/command/internal/CollisionNotice.java`

Package-private enum `{NONE, INFO, WARN}` that decides and words the report of a collided root or alias key, free of Bukkit types so the decision and the exact texts are unit-tested without a server (`CollisionNoticeTest`).

- `static CollisionNotice of(boolean dynamic, boolean ownsNamespace, boolean alreadyNoted)` - `WARN` when the root is not `dynamic()` or the `prefix:key` form is someone else's too (unreachable); otherwise `INFO` the first time and `NONE` once noted.
- `static String occupant(@Nullable String pluginName)` - the name, or `UNKNOWN_OCCUPANT` (`"another command"`) for null/blank.
- `static String rootInfo(name, ownerName, occupant, prefix)` - `Command '/<name>' of <Owner> is taken by <occupant>; kept it. This command answers as /<prefix>:<name>`.
- `static String rootWarn(name, ownerName)` - the unchanged pre-1.38.0 `Command '/<name>' of <Owner> collides with an existing command; kept the existing one`.
- `static String aliasInfo(alias, rootName, ownerName, occupant, prefix)` - `Alias '/<alias>' of '/<root>' in <Owner> is taken by <occupant>; kept it. This alias answers as /<prefix>:<alias>`.
- `enum Occupant { OWN, SERVER, PLUGIN, SNLIB_ROOT, SNLIB_PRIORITY_ROOT }` (1.39.0) - who holds a key a root wants, as `BukkitCommandRegistry.occupantKind` classifies it.
- `static boolean takesOver(boolean priority, Occupant occupant)` (1.39.0) - `priority && (occupant == PLUGIN || occupant == SNLIB_ROOT)`.
- `static String rootTakeover(name, ownerName, occupant, @Nullable occupantPrefix)` (1.39.0) - `Command '/<name>' of <Owner> took the name from <occupant> (command priority); <occupant> still answers as /<occupantPrefix>:<name>`, the clause after `;` only with a non-null prefix.
- `static String aliasTakeover(alias, rootName, ownerName, occupant, @Nullable occupantPrefix)` (1.39.0) - `Alias '/<alias>' of '/<root>' in <Owner> took the name from <occupant> (command priority); <occupant> still answers as /<occupantPrefix>:<alias>`, same rule.

#### CommandPriorityListener (internal, 1.39.0)
`src/main/java/com/sn/lib/command/internal/CommandPriorityListener.java`

Shared listener, inscribed in `ListenerHub` (the only `registerEvents` point), with two `@EventHandler(priority = EventPriority.MONITOR)` methods, `PluginEnableEvent` and `ServerLoadEvent` (startup and reload), calling `BukkitCommandRegistry.reclaimPriority(event.getPlugin())` and `reclaimPriority()`. `PluginEnableEvent` fires after the plugin's `onEnable`, and Paper registers a plugin's plugin.yml commands right before it, so a later plugin that overwrote an alias key of a priority root loses it again at the end of its own enable. With no priority root registered the pass is a walk over the registered roots and nothing else.

#### Command priority (1.39.0)

What `SnSpec.Builder.commandPriority()` changes, end to end:
- **Taken** (`CollisionNotice.takesOver`): the bare name (fallback path and declared path) and every alias of every root of the plugin, when their holder is a command another plugin provides (`PLUGIN`: its plugin.yml command, or any `Command`/`BukkitCommand` class of its own; a disabled plugin's included, its command can no longer run) or a root of another SnLib plugin without priority (`SNLIB_ROOT`). The holder keeps its namespaced `plugin:key` form, which is never touched.
- **Never taken** (the 1.38.0 INFO/WARN applies): the plugin's own keys (roots of the same plugin sharing an alias), classes of the server itself (vanilla, Bukkit and Paper commands: `version`, `plugins`, `reload`, `help`, `timings`, `PaperCommand`, `MSPTCommand`, `SpigotCommand`, `TicksPerSecondCommand`, `RestartCommand`, `commands.yml` aliases), every Paper Brigadier command (a `PluginVanillaCommandWrapper`, native or registered by a plugin through the lifecycle API), any key holding `:` and a root of another plugin with priority (whichever holds the key keeps it, so two cores never trade it back and forth).
- **INFO** once per owner and key per enable: `Command '/money' of SnDungeons took the name from Essentials (command priority); Essentials still answers as /essentials:money` and `Alias '/bal' of '/money' in SnDungeons took the name from Essentials (command priority); Essentials still answers as /essentials:bal`; the clause after `;` only when the displaced command still holds its namespaced key, otherwise the line ends at `(command priority)`.
- **Given back** when the root releases the key: the plugin disables (the tenant sweep), a reload drops the root or the alias, a root of the same name replaces it (then the new root takes the key again in the same pass, silently), or `unregister` (then the owner's other priority roots take back what they want first). Never while the server is stopping, never over a key someone else took meanwhile, and only to a command that can run there (`returnable`); a plugin that re-registered its commands or was reloaded by a plugin manager gets it through its live `plugin:key` form. A plugin that enabled after the root and was refused the key is recorded as displaced at its `PluginEnableEvent` (`noteLatecomer`) and gets the key too.
- **Taken back** from a later plugin by `CommandPriorityListener` (a plugin finished enabling, the server finished loading) and on every register pass of the owner (its reload).
- **Limitations:** the native Brigadier command of another plugin (`PluginVanillaCommandWrapper`) is NOT taken, by design: it is a command of Paper's Brigadier API, not a Bukkit plugin command; a plugin that registers commands in a later tick than its enable is taken back only at the next `PluginEnableEvent`, `ServerLoadEvent` or owner reload; `plugin.yml` `aliases:` of a declared root are Bukkit's and never taken (declare the aliases through the builder or `aliasesFromConfig()`); a root of another SnLib plugin replaced by a new instance while displaced gets its name back, and an alias back only when the new instance holds that alias's namespaced form (otherwise at its own next register pass); a command class loaded from a plugin's `libraries:` is not attributed to the plugin (`getProvidingPlugin` refuses it) and stays `SERVER`; a command class of a command framework installed as its own plugin, created by another plugin without `PluginIdentifiableCommand`, is attributed to the framework (taken, but never given back since it does not answer at `<framework>:<key>`); a priority consumer's DECLARED root is recognised as priority only once it registered it (until then its `PluginCommand` has no adapter and counts as `PLUGIN`); a command registered under a fallback prefix other than its plugin's name is never given back (it does not answer at `<plugin>:<key>`); a plugin that enabled after the priority plugin is recorded as displaced only for the keys the root held at that plugin's `PluginEnableEvent`; between two priority plugins Bukkit's own rule decides who holds an alias key (a later plugin.yml LABEL overwrites an earlier alias), and the earlier one does not take it back; while a root holds the bare name of a plugin.yml command, `Bukkit.getPluginCommand(name)` returns null to other plugins and that entry's `permission`/`usage` do not apply to the bare label (the root's own permission does; the owner's `getCommand(name)` still finds the declared command through its namespaced key). The server owner's way out is renaming the command or alias in the priority plugin's config.

#### AliasReconciler (internal, 1.5.0)
`src/main/java/com/sn/lib/command/internal/AliasReconciler.java`

Pure alias reconciliation helpers, free of Bukkit types so the decision layer is unit-testable without a running server. Source policy: a non-null `supplied` collection is AUTHORITATIVE (an empty list means "no aliases"); a null `supplied` means the source has no opinion and the `fallback` (builder / plugin.yml aliases) applies. Every resolved alias is trimmed, lowercased (`Locale.ROOT`), de-duplicated in encounter order, and stripped of the root name and of the plugin.yml declared aliases (owned by Bukkit, not by this dynamic layer). The same source policy decides the plugin.yml WARN (1.12.1).
...
- `static boolean warnsUndeclared(@Nullable Collection<String> supplied, Collection<String> added)` (1.12.1) - whether the added aliases deserve the "not declared in the plugin.yml" WARN: true only when `supplied` is null (fallback path) AND something was added. A non-null `supplied` means an authoritative source owns the aliases at runtime, so declaring them in the plugin.yml is impossible and the WARN would be boot noise. Since 1.38.0 it delegates to the overload below with `dynamic = false`.
- `static boolean warnsUndeclared(boolean dynamic, @Nullable Collection<String> supplied, Collection<String> added)` (1.38.0) - `!dynamic && supplied == null && !added.isEmpty()`: a `dynamic()` root is never declared in the plugin.yml, so neither are its aliases, whatever their source.
...
- Dynamic aliases are claimed, never overwritten: if another command already owns the alias key in the CommandMap, the alias is not operative for the root under that bare key and the collision is reported as a WARN (for a `dynamic()` root that still owns `prefix:alias`, one INFO per enable, 1.38.0). Since 1.33.1 a collided alias is NOT recorded as active, so the next register pass retries it (an alias whose owner disappeared starts working after a reload) and re-warns while it stays taken.
- `dynamic()` collisions are noted once per owner and key PER ENABLE (1.38.0): a collision that clears and comes back within the same enable is not noted again; the next enable (after `Sn.shutdown` swept `COLLISIONS_NOTED`) notes it afresh. Command priority takeovers (1.39.0) follow the same rule under their own mark.
- Command priority (1.39.0) cannot take a Brigadier command of another plugin (its wrapper cannot be put back), re-takes a key a plugin overwrote in a later tick only at the next `PluginEnableEvent`/`ServerLoadEvent`/owner reload, and never takes the plugin.yml `aliases:` of a declared root; see "Command priority (1.39.0)" above.
- Help reached through the namespaced form shows the bare name (known limitation, left for a follow-up): `typedLabel` strips `plugin:` from the label, so `/sndungeons:money help` renders `/money ...` lines even when `/money` belongs to another plugin whose command would run instead.
- Dynamic aliases must be written through the CommandMap's own `get`/`put` (1.33.1): on Paper 1.20.6+ `getKnownCommands()` returns `BukkitBrigForwardingMap`, a `HashMap` subclass that forwards its OVERRIDDEN methods to the Brigadier dispatcher. The inherited ones do not forward: `putIfAbsent` writes into the dead backing table and returns null (reporting a successful, non-colliding insert), and the entry-set iterator streams the dispatcher and refuses `remove`, so a bulk `entrySet().removeIf` throws `UnsupportedOperationException` as soon as a predicate matches. This is what made every config-driven alias silently non-dispatchable, in its bare and its `plugin:alias` form, from 1.20.6 onwards; the same trap applies to any consumer that touches `getKnownCommands()` directly.
- Registering after boot needs a dispatcher sync on the OLD generations (1.33.1): on Paper 1.20.4 / 1.20.5 the known commands are a plain map and `CraftServer#syncCommands()` is what copies them into the dispatcher - the server calls it once at the end of `enablePlugins(POSTWORLD)`, so cold boot works and the reload path would not. The internal `CommandSync` invokes it reflectively (walking up from the concrete server class, so no package or NMS version string is hardcoded) after any pass that actually mutated the map, and degrades to the `player.updateCommands()` refresh when the server exposes no such method (reported once at FINE). On Paper 1.20.6+ that same call only re-sends the client trees, which is all it needs to do there.
- Config-driven aliases (1.5.0): `aliasesFromConfig()` reads the list at `command.aliases`; when the key is set it is authoritative (an empty list clears the aliases), re-sourced on every reload. `aliases(Supplier)` is the code-level equivalent. `aliases(String...)` / plugin.yml aliases are the fallback. `aliasesFromConfig()` needs the config module; without it, or with the key absent, the fallback applies. A plugin.yml `aliases:` entry is always stripped from the dynamic set (Bukkit owns those and binds them at load): a consumer that wants config.yml to hold FULL authority - including the power to remove an alias - must leave the key out of its plugin.yml.
---
...
- `createDataSource()` (both profiles, 1.24.0): Hikari's `connectionTimeout` is set from `config.connectTimeoutSeconds() * 1000` BEFORE the backend branch, so neither backend can block on a connect for longer than the configured budget (previously unset, i.e. Hikari's 30s default). When that budget is shorter than HikariCP's own 5s validation default, `validationTimeout` follows it down (`HIKARI_DEFAULT_VALIDATION_TIMEOUT_MILLIS = 5000`), so the liveness check cannot outlive the borrow either; a config that asked for nothing keeps Hikari's value untouched. The budget only ever measures the PHYSICAL connect: the executor has exactly as many threads as the pool has connections (1 for SQLite, `poolSize` for MySQL), so a borrow never queues behind another borrow and contention cannot consume it.
...
- `createDataSource()` (MySQL profile): driver `com.mysql.cj.jdbc.Driver`, URL from `mysqlUrl(...)`, user/password, `maximumPoolSize=max(1, poolSize)` (default 4), and prepared statement caching (`cachePrepStmts=true`, `prepStmtCacheSize=250`, `prepStmtCacheSqlLimit=2048`).
- `static String mysqlUrl(String host, int port, String database, boolean ssl, long connectTimeoutMillis, long socketTimeoutMillis)` - (1.24.0) package-private and static so the parameter assembly is unit-tested (`DbTimeoutTest`): renders `jdbc:mysql://host:port/database?useSSL=<ssl>&allowPublicKeyRetrieval=true&characterEncoding=utf8&connectTimeout=<ms>&socketTimeout=<ms>`. The first three parameters are exactly what the URL carried before; the last two are the addition. `socketTimeout=0` means unlimited (the driver default), which is the shape that hangs forever on a black-holed link once the connection is already open.
...
- The lazy pool creation happens INSIDE the first `submit`, so the connect budget is paid on the db executor and surfaces as an exceptionally-completed future, never as a main-thread stall. With Hikari's default `initializationFailTimeout`, `new HikariDataSource(...)` opens the first connection there and now fails within `connect-timeout-seconds` instead of Hikari's 30s default - which is what makes `bootstrap(...).orDisablePlugin()` against a dead host resolve in seconds.

### DbConfig
`src/main/java/com/sn/lib/db/DbConfig.java`

A consumer's database connection settings, parsed from the `database` section of its main config. Recognized keys: `type` (sqlite or mysql, default sqlite), `file` (SQLite path relative to the data folder, or absolute), and for MySQL `host`, `port`, `database`, `username`, `password`, `pool-size` and `ssl`, plus the two timeout keys below. An absent section or an unknown `type` fall back to SQLite at `<dataFolder>/database.db`.

Timeout keys (1.24.0, both optional and both read from the same `database` section, so every existing config keeps working untouched): `connect-timeout-seconds` (default 10, clamped to 1..3600) caps opening a connection and `socket-timeout-seconds` (default 30, clamped to 0..3600, where 0 = unlimited) caps a read over an open one. They exist so an unreachable host cannot outlive the caller's own budget - a bounded `SnFuture.joinWithin` is worthless if the block happens below the future, inside the driver.

Public enum:

- `public enum Type { SQLITE, MYSQL }` - supported backends.

Constants (private, they define the defaults): `DEFAULT_SQLITE_FILE = "database.db"`, `DEFAULT_MYSQL_PORT = 3306`, `DEFAULT_MYSQL_POOL_SIZE = 4`. Package-private (1.24.0, read by `DbTimeoutTest`): `DEFAULT_CONNECT_TIMEOUT_SECONDS = 10`, `DEFAULT_SOCKET_TIMEOUT_SECONDS = 30`, `MAX_TIMEOUT_SECONDS = 3600`.

Public methods:

- `public static DbConfig load(JavaPlugin plugin, @Nullable ConfigurationSection section)` - parses the `database` section; a null section yields the SQLite defaults. An unknown `type` logs a WARN (`"invalid database.type: '<raw>', using sqlite"`) and falls to SQLite. The `database` default (MySQL) is the lowercased plugin name; `username` defaults to `root`, `password` defaults empty, `pool-size` clamps to a minimum of 1, `ssl` defaults to false, and the two timeouts go through `clampConnectTimeout` / `clampSocketTimeout`.
...
- `public int connectTimeoutSeconds()` - (1.24.0) seconds a single connection attempt may take; applied as Hikari's `connectionTimeout` on BOTH backends and as the MySQL driver's `connectTimeout` on the URL.
- `public int socketTimeoutSeconds()` - (1.24.0) seconds a read over an already-open MySQL connection may block; applied as the driver's `socketTimeout`, `0` meaning unlimited. SQLite ignores it, being a local file.

Package-private methods (clamps, unit-tested):

- `static int clampConnectTimeout(int seconds)` - to 1..`MAX_TIMEOUT_SECONDS`; it can never be zero, since Hikari rejects a connection timeout under 250 ms and "unlimited" is exactly the state this key exists to prevent.
- `static int clampSocketTimeout(int seconds)` - to 0..`MAX_TIMEOUT_SECONDS`; 0 is kept as the explicit opt-out for a consumer whose queries legitimately run longer than the cap.
...
- `public static <T> SnFuture<T> wrap(Sn ctx, CompletableFuture<T> future)` - (v1.1) wraps an arbitrary `CompletableFuture` in SnFuture's consumption surface (`thenSync`/`exceptionally`/`join`) of the given context; used by library modules outside the db package and available to consumers. The resulting SnFuture has no origin `SnDb` (null db), so the join WARN is never suppressed by a bootstrap phase. Prefer `wrapMainCompleted` whenever only a main-thread task can complete the future, which is what `SnPapi.applyOnMain` returns from its off-main branch (1.27.0).
- `public SnFuture<T> thenSync(Consumer<T> consumer)` - consumes the value on the main thread via the owner's scheduler; the hop is skipped when the owning plugin is already disabled (the scheduler's is-enabled guard), and a failed future logs a WARN instead of reaching the consumer. Returns `this`, so it does NOT extend a chain: a continuation registered on the returned future is a SIBLING of this consumer, not a successor. Use `chainSync` when the future is returned to a caller who may chain onto it.
- `public SnFuture<T> chainSync(Consumer<T> consumer)` - (1.27.0) the same main-thread hop, but returns a NEW future completed from INSIDE that main-thread task once the consumer returned, which is the only way on this surface to express "B after A's continuation". It exists because `thenSync`, `exceptionally` and `orDisablePlugin` all `return this`: a method ending in `return write(...).thenSync(publish);` whose caller writes `callee().thenSync(next)` registers two dependents on ONE `CompletableFuture`, and since sibling order is unspecified and OpenJDK pops dependents last-registered-first, `next` is scheduled BEFORE `publish` and reads the state the publish was about to install. Hopping to the main thread does not rescue it: both tasks are then queued in that inverted order. The fix belongs in the PRODUCER - swapping its one `thenSync` for this makes every existing caller correct without touching them. Three deliberate divergences from `thenSync`: a source failure is PROPAGATED to the returned future rather than swallowed into a WARN (so terminate the chain with `exceptionally`/`orDisablePlugin`, or the failure is seen by nobody); a consumer that THROWS fails the returned future rather than only reaching Bukkit's task reporter (a link that never settles is a permanent hang for whoever waits above it); and a skipped hop (plugin disabled, `ctx.isShuttingDown()`, or the `IllegalPluginAccessException` disable race, which logs WARN `"Chained hop to main discarded: plugin disabled during scheduling"`) completes the returned future NORMALLY with the value, so a teardown never becomes a hang - meaning a normal completion does NOT prove the consumer ran. Any other scheduling throwable fails the future instead of passing as a clean skip (Folia's global-region scheduler is not bound to the Bukkit exception type). The derived future carries `mainThreadCompleted` so `join()`/`joinWithin` throw from the main thread while it is pending instead of deadlocking - but only when the plugin is enabled and the context is not already tearing down AT THE MOMENT chainSync is called, because from then on the hop can never be allowed and the future settles on the completing thread instead. Marking it unconditionally would fire the guard during a teardown flush on a wait that would have returned, and an `IllegalStateException` escaping `onInnerDisable` takes the rest of that flush with it. The residual window (a teardown that STARTS after the call) is documented, not closed. Null consumer throws `NullPointerException` eagerly on the calling thread. Sequencing core split into the package-private `static <T> void chainStep(...)` with the hop and the log injected, so all branches are unit-tested without a server (`SnFutureChainTest`, 8 tests).
...
- `public T join()` - blocks until the value is available and returns it. Intended ONLY for the shutdown flush and the enable bootstrap: any other main-thread join (an uncompleted future, outside `ctx.isShuttingDown()` and `db.inBootstrap()`) logs a WARN `"SnFuture.join() on the main thread outside shutdown/bootstrap:"` with the first 5 caller frames. The shutdown window covers the consumer's own `onInnerDisable()`, not just the 13 teardown steps: `SnPlugin.onDisable()` calls `beginTeardown()` first, so a consumer flushing its data with a `join()` there is silent (as it always should have been - before 1.20.2 that legitimate flush tripped the WARN). The wait itself is unbounded.
- `public boolean joinWithin(Duration timeout)` - (1.24.0) the same wait under a budget: `true` if the future settled before the timeout, `false` if the budget ran out with the work still in flight. Exceptional completion behaves EXACTLY like `join()` and throws (`CompletionException` wrapping the cause, unwrapped from the `get`'s `ExecutionException`; `CancellationException` when cancelled), so a throw always means "the operation failed" and never "it is still running" - the three outcomes are distinguishable without inspecting the future. A timeout neither cancels nor completes it: the work keeps running and a late failure still needs an `exceptionally` registered beforehand to be observed; after a `true` the value is available from `join()`, which returns at once on a settled future. An interrupt re-arms the interrupt flag and returns `false`. A null timeout throws `NullPointerException`; a zero or negative one polls once; a `Duration` too large for `toNanos()` degrades to `Long.MAX_VALUE` nanoseconds instead of throwing. Main-thread rules carry over unchanged: the same shutdown/bootstrap suppression (the WARN reads `"SnFuture.joinWithin() on the main thread outside shutdown/bootstrap:"`) and the same `wrapMainCompleted` refusal, since a budget would postpone that deadlock rather than break it. It exists because Bukkit clears the enabled flag BEFORE `onDisable`, so every `thenSync` continuation is dropped from the first line of `onInnerDisable` onward: blocking is the only way left to observe a teardown write, and an unbounded block on one unreachable database holds the server stop open for as long as the JDBC driver takes.
...
- The join WARN is suppressed in four cases: an already-completed future, a non-main thread, a shutting-down context, or the origin `SnDb`'s bootstrap phase (`db != null && db.inBootstrap()`; `wrap` futures have no db). The stack trims starting at frame 3 to skip the internal `getStackTrace`/`join` frames. `join()` and `joinWithin(...)` share `warnIfMainThreadJoin(String method)` and `failIfMainCompletedOnMain(String detail)`, each passing its own name; both call them directly, so the frame-3 trim is right for either entry point and `join()`'s two messages are byte-for-byte what they were before 1.24.0.
- A bounded wait is only half a bound: without the JDBC timeouts of `DbConfig` (1.24.0) a single `getConnection()` against a black-holed host outlasts any budget the caller sets, because the block happens below the future. `joinWithin` and those keys are one feature.
- **Never wait on a chained future.** A `chainSync` result can only complete normally from inside a main-thread task, so a main-thread wait throws (the `wrapMainCompleted` guard turns a silent server deadlock into an immediate failure) and an ASYNC wait is unsafe in exactly the place it matters: during a stop the main thread sits inside `SnDb.flushPlayerCaches`, so nothing will ever run the hop and the guard cannot catch it (it only fires on the main thread). A producer whose future a teardown flush joins must therefore NOT adopt `chainSync` - once it does, the method returns only the derived future and the joinable source is unreachable. Joins belong on the future the db module returned (`PlayerDataCache.saveAll` keeps returning a `fence()` future on purpose).
- `chainSync` settles on every path it can reach except one: a hop that passed the is-enabled check, got queued, and was then cancelled by a disable (a server stop, or ANY `disablePlugin` including `/reload`). The returned future stays pending for the rest of the run. Nothing downstream executes, which is exactly what `thenSync` would have done in that window, but a wait on it never returns. Documented rather than closed: closing it needs a per-context registry of pending chained futures drained by `Sn.shutdown()` after `scheduler.cancelAll()`, which is a behaviour change to an existing method plus a new retained-object set. Asserted as a known residual by `SnFutureChainTest.documentsTheOneWindowThatDoesNotSettle`.
- Swapping a producer's `thenSync` for `chainSync` retroactively changes the JOIN contract of every existing caller of that producer, because the returned future becomes `mainThreadCompleted`. Audit every `join()`/`joinWithin` on that method's result before the swap; it is a one-word edit with a non-local precondition.
...
- `finalName`: `SnLib-${project.version}` (produces `SnLib-<version>.jar`, e.g. `SnLib-1.39.0.jar`).
...
- `player-inventory` (1.28.0, `locked` | `open`, default `locked`): with `locked` every click and drag over the viewer's OWN inventory is cancelled while the menu is open, exactly as before 1.28.0. With `open` the viewer uses their inventory normally (plain clicks, number keys, Q, F, drags inside it) while clicks over the menu's cells and the double-click gather stay cancelled, and a shift-click on one of their stacks is reported to the plugin as an offer instead of moving the item. Unknown values WARN and fall back to `locked`. The key governs the BOTTOM inventory only; which CELLS accept an item is the per-item `input:` key.
- `strict-clicks` (opt-in per menu, default false): with `true` the generic `click-actions` list only fires on the 4 basic mouse clicks (LEFT, RIGHT, SHIFT_LEFT, SHIFT_RIGHT); the other ClickTypes stay cancelled with no action, unless a declared specific list covers them (`middle-click-actions` enables MIDDLE; a declared `left-click-actions` enables DOUBLE_CLICK and CREATIVE; a declared `drop-click-actions` enables DROP and CONTROL_DROP since 1.31.0). NUMBER_KEY, SWAP_OFFHAND and UNKNOWN have no specific list to declare and stay discarded whatever the item says. With `false` (default) the behavior is the historical v1.0.0 one.
...
- The `items` schema: `display-name`, `material` (resolves basehead-base64), `custom-model-data`, `item-model` (1.21.2+ base ItemModel key), `amount`, `slots` (int, a "0-2" range or a "0, 2, 4-6" mix), `glow`, `enchantments` (id/level pairs), `flags` (HIDE_ENCHANTS, HIDE_ATTRIBUTES, HIDE_UNBREAKABLE, HIDE_DESTROYS, HIDE_PLACED_ON, HIDE_POTION_EFFECTS, HIDE_ALL as the combination, and HIDE_TOOLTIP since 1.29.0 for the whole tooltip box including the display name - a separate flag, never implied by HIDE_ALL), `color` (RGB "235, 64, 52" or HEX), `trim-pattern`/`trim-material` (armor), `potion-effects` (effect/level/duration triples), per-item `update-interval`, `input` (1.28.0, default false: the cell RECEIVES an item - a click with a stack on the cursor or a drag over it is reported to the plugin instead of firing the click actions, while a click with an EMPTY cursor still fires them, so one cell is a button and a drop target at once; documented with the example item `example-input-item`, whose other half is the ONLY thing in this golden spec that needs plugin code, and which WARNs on load unless `player-inventory: open` is set), `lore`, `click-requirements` and `view-requirements` (expressions `%placeholder% > 0 && %placeholder% < 10`, `=`, `!=`), `deny-actions` (same actions, they run if the click-requirements are NOT met) and `click-actions`. Since 1.17.0 several items may declare the same slot (or the same layout `key:`): candidates resolve in declaration order and the first whose `view-requirements` pass for the viewer owns the cell (all hidden = empty cell), so mutually exclusive variants of one button are declared as sibling items, no priority field.
- The per-click matrix (1.1.0; the `drop` key since 1.31.0): 18 optional keys per item, six click keys (`right`, `left`, `shift-right`, `shift-left`, `middle`, `drop`) times three lists (`*-click-actions`, `*-click-requirements`, `*-click-deny-actions`), with the specific-over-generic field-by-field resolution rule documented in the spec (exact shift -> side -> drop -> generic; right covers RIGHT/SHIFT_RIGHT, left covers LEFT/SHIFT_LEFT/DOUBLE_CLICK/CREATIVE, middle covers MIDDLE, drop covers DROP and CONTROL_DROP - the Q key, the only keyboard click of the matrix).
...
- The `templates` section: items IDENTICAL to normal ones with `slots:`/`key:` OPTIONAL instead of required (1.18.0; before, templates could not declare placement at all). Placement comes from either side: the template declares `slots:` or `key:` against the menu `layout:` and the plugin binds it slotless (`session.bind("template-id", phs...)`), so the server owner repositions the element by editing the file; or the template declares neither and the plugin decides per bind (`session.bind(slot, template, phs...)`, which always ignores the declared cells - per-entry lists keep working). The config user customizes the appearance freely; templates can use plugin-defined local placeholders (e.g. `%index%`, `%warp_name%`) and support the same per-click matrix keys as regular items. It includes the usage example of a backpack plugin (the plugin assigns per player which template goes to which slot).
...
- APPEARANCE: `display-name`, `material` (resolves basehead-base64), `custom-model-data`, `item-model` (1.21.2+ base ItemModel key, independent of custom-model-data), `amount`, `glow`, `lore`, `enchantments`, `flags` (same set as menus, with HIDE_ALL and HIDE_TOOLTIP), `color` RGB/HEX, `trim-pattern`/`trim-material`, `potion-effects`.
...
14 tests over the per-click matrix of `com.sn.lib.gui.GuiItemDef`: the package-private pure mappings that support resolution and the strict-clicks gate (`shiftKey`, `sideKey`, `dropKey`, `basicClick`) plus, since 1.31.0, the resolution itself over items parsed from REAL yml text. Pure JVM: the fixtures go through the production `GuiItemDef.parse` over a temp file read by a context-less `SnYml` (`SnYmlTestAccess`), so a test proves the yml key is read and not merely that a hand-built definition resolves.
...
- `void sideKeyNullForKeyboardAndUnknownClicks()` - NUMBER_KEY, DROP, CONTROL_DROP, SWAP_OFFHAND and UNKNOWN have no side (null: DROP/CONTROL_DROP resolve their entry through `dropKey` instead, the rest fall to the generic and in strict stay discarded).
...
- `void dropKeyMapsBothDropClicksAndNothingElse()` (1.31.0) - DROP and CONTROL_DROP map to `ClickKey.DROP`; the sweep over the whole `ClickType` enum plus null asserts every other click maps to nothing, so NUMBER_KEY/SWAP_OFFHAND/UNKNOWN cannot quietly join the matrix.
- `void aDeclaredDropListWinsOverTheGenericOneForBothDropClicks()` (1.31.0) - `drop-click-actions` resolves for DROP and CONTROL_DROP and makes `specificActionsFor` true for both, while LEFT/RIGHT/SHIFT_LEFT/MIDDLE keep returning the very same generic list instance.
- `void dropRequirementsAndDenyActionsResolveLikeEveryOtherKey()` (1.31.0) - `drop-click-requirements` and `drop-click-deny-actions` win for both drop clicks and leave the other clicks on the generic trio.
- `void eachDropFieldFallsBackToTheGenericOneOnItsOwn()` (1.31.0) - an item declaring only `drop-click-actions` still takes its DROP requirement and deny list from the generic fields.
- `void anItemWithoutDropListsResolvesExactlyAsItDidBefore1310()` (1.31.0) - the compatibility claim of the release: with no drop list declared, DROP and CONTROL_DROP resolve the generic actions, requirement and deny list and `specificActionsFor` is false.
- `void strictClicksLetADeclaredDropThroughAndKeepDiscardingAnUndeclaredOne()` (1.31.0) - the gate predicate (`!basicClick && !specificActionsFor`, mirrored from `GuiSession.runClick`) passes a declared drop and discards an undeclared one.
- `void strictClicksStillDiscardTheOtherKeyboardClicksWithADropListDeclared()` (1.31.0) - NUMBER_KEY, SWAP_OFFHAND and UNKNOWN stay discarded even on an item that declares the drop list: the explicit non-goal.
- `void paginationTagsAreDetectedInTheDropListToo()` (1.31.0) - `detectNav` scans the drop entry like the other five, so `[next-page]`/`[previous-page]` in `drop-click-actions` make a navigation item.
...
- Dedicated-repo mode (`checkLatest`): GET `https://api.github.com/repos/<repo>/releases/latest`. Shared-repo mode (`tagPrefix` non-null/non-empty, `checkShared`): the list endpoint `https://api.github.com/repos/<repo>/releases?per_page=100&page=<n>` instead, read through `com.sn.lib.update.internal.ReleaseFeed` (the paged shared read described below). Same headers either way: `Accept: application/vnd.github+json`, `X-GitHub-Api-Version: 2022-11-28`, `User-Agent: SnLib-UpdateChecker`; timeouts 5s connect / 10s request (the same values as DiscordWebhook), a lazy per-instance HttpClient with double-checked locking (the exact `DiscordWebhook.client()` pattern), synchronous `send` (it already runs on an async thread).
...
- **The feed is PAGED and SHARED** (`ReleaseFeed`): a GitHub releases page holds at most 100 releases OF THE WHOLE REPO, so on a shared repo a single request stops covering every plugin as soon as the repo passes 100 releases - the plugins whose newest release fell out of that window report a missing prefix, and which ones those are drifts with every release anyone publishes (the 1.34.3 bug: `Sn-Releases` at 203 releases hid 16 prefixes). `matching(repo, prefix, token)` walks pages from 1 until a short page (`size() < 100`, the last one) or `MAX_PAGES = 10`, returning a `Scan(matches, truncated, scanned)`. Paging multiplies requests against a 60/hour unauthenticated IP budget, so each page is fetched ONCE PER SERVER and shared by every consumer of that repo: a `ConcurrentHashMap` of parsed pages keyed `repo + page + tokenId` with a 5-minute TTL (spans a boot's staggered first checks and each 6h cycle), plus an IN_FLIGHT `CompletableFuture` per key so concurrent readers wait on the one request rather than firing their own. Cost per cycle: ONE request per page of the repo whatever the plugin count. Only the parsed `(tag, url)` pairs are retained, never the raw bodies (a page is hundreds of KB). A failure is never cached; `tokenId` is the token's hash, so no raw secret sits in a long-lived key. Server-wide statics justified (a feed is content-addressed and identical for every consumer); `ReleaseFeed.clearCache()` runs in `SnLibPlugin.onDisable` beside `PlayerLookup.clearCache()`, and it owns its own lazy static HttpClient - so consumers on a shared feed no longer each build one.
- Parsing without a JSON library: the package-private helper `static @Nullable String jsonString(String body, String field)` (a hand scanner: the first occurrence of quoted `"field"`, skips spaces and `:`, demands an opening `"` and reads up to the closing one un-escaping `\"`, `\\` and `\/`; an unexpected format returns null; documented assumption: in the releases/latest payload the first occurrence of `html_url` is the release's) - both `jsonString` and the new list scanner share the underlying `stringValueAt(body, index, field)` reader. For shared-repo mode, `ReleaseFeed.parseReleaseTags(String body)` (which carries its own copy of `stringValueAt`, following the self-contained-utils precedent of `PlayerLookup`) scans a `releases` array and pairs every `tag_name` with the nearest PRECEDING `html_url` (safe because each release object emits its own `html_url` once, before its `tag_name`, and asset entries carry `url` but never `html_url`). And `static String stripTagPrefix(String tag)`: trim + strips the leading `v`/`V` ONLY if a digit follows (`v1.4.0` -> `1.4.0`, `vanilla` intact) - applied to a matched tag AFTER the plugin's `tagPrefix` is stripped off in shared mode.
- Shared-mode selection: among `ReleaseTag`s whose `tag()` starts with `tagPrefix`, the prefix is stripped, then `stripTagPrefix`, then the highest version wins via `SemverComparator.compareVersions`. No match -> warn-once "no release tag matching prefix '<prefix>'", which means the prefix is absent from the WHOLE repo; when the walk stopped at the `MAX_PAGES` ceiling instead, the message ends "... in the newest <scanned> releases", so a missing prefix is never confused with a repo too large to scan.
...
- Handoff consistency note: the handoff mentions "114 tests"; the last count verified in this documentation was 204 tests across 21 suites (see prior revisions for the full step-by-step history). Current verified count (surefire, `mvn test`) is 667 tests, all green, on the 1.39.0 release.
...
- **`ownJar()` resolves the INSTALLED jar, not the loaded one (fixed in v1.16.2).** Paper 1.20.5+ remaps every plugin at boot and loads the rewritten copy out of `plugins/.paper-remapped/`, so `getProtectionDomain().getCodeSource()` - which 1.16.0/1.16.1 trusted outright - points INSIDE that cache on any modern server. The swap then wrote `SnLib-<new>.jar` into the remap cache, deleted the cached `SnLib-<old>.jar`, never touched `plugins/SnLib-<old>.jar`, and still announced "installed on disk": the cache is regenerated from `plugins/` on the next boot, so the server came back on the old version with no error anywhere. The resolution is now: take the code source ONLY when `isChildOf(loaded, pluginsDir)`; otherwise try `pluginsDir/<same file name>` validated by `isOwnJar`; otherwise scan `plugins/*.jar` for the single jar whose descriptor declares `name: SnLib` + `main: com.sn.lib.SnLibPlugin`, returning null (WARN, no swap) when more than one matches. The plugins folder itself comes from `Bukkit.getUpdateFolderFile().getParentFile()`, which the server derives from the plugins directory it was started with, so a non-default plugins path still resolves correctly.
- Because a broken updater cannot fix itself, **every server on 1.16.0/1.16.1 that remaps plugins needs ONE manual SnLib upload** to reach 1.16.2; self-update works normally from there.
